...
- System-admin list/grant/revoke APIs require active dedicated system-admin access and refuse to remove the last active system admin; production-like grant/revoke additionally requires recent authentication inside the configured step-up window. Workspace security policy APIs require workspace `workspace.admin`, and project security policy APIs require project `project.admin`. Audit log, audit retention, and domain event replay APIs are workspace-admin-only through method-level permission checks.
