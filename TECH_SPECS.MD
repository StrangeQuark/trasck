...
- `GET /api/v1/workspaces/{workspaceId}/roles` lists workspace-scoped role picker options for callers with `user.manage`; `GET /api/v1/projects/{projectId}/roles` lists project-scoped role picker options for project admins.
...
- System-admin list/grant/revoke APIs require active dedicated system-admin access and refuse to remove the last active system admin; production-like grant/revoke additionally requires recent authentication inside the configured step-up window. Workspace security policy APIs require workspace `workspace.admin`, project security policy APIs require project `project.admin`, workspace role picker reads require `user.manage`, and project role picker reads require project `project.admin`. Audit log, audit retention, and domain event replay APIs are workspace-admin-only through method-level permission checks.
