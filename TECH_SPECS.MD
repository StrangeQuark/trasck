...
67. A full production-minded security audit has been recorded in `SECURITY_AUDIT.MD`. The current hardening baseline removes tracked `.env` files, adds committed examples only, rejects unsafe production defaults, requires Redis-backed auth throttling in production-like profiles, gates Swagger/OpenAPI behind dedicated system-admin access in production-like profiles, exposes system-admin grant/revoke/list management with production-like recent-login step-up on grant/revoke, enforces explicit CORS, adds outbound egress/SSRF controls with exact host, wildcard host, and CIDR allowlisting, tightens login/password policy, persists auth-failure telemetry, rate-limits agent callback/worker auth failures, enforces workspace-admin and project-admin configurable attachment/import/export size and content-type policy, masks frontend secret displays, centralizes redacted API error bodies/log messages while preserving explicit client-error status shapes, templates production nginx CSP, wires Java dependency vulnerability scanning as a required CI gate, and adds targeted regression tests. Remaining production hardening should focus on stronger secret-management/KMS abstraction, Redis connectivity/health packaging, step-up UX hardening, and broader redaction/tenant-isolation regression coverage. Production secrets must be injected through environment variables or local `.env` files that are not committed; checked-in config may contain only safe local examples.
...
- New human passwords are validated for configured minimum length and common development password rejection during setup, invitation registration, and workspace-admin direct user creation. Login failures are throttled by identifier plus remote address with configurable database-backed or Redis-backed counters, failure window, and lockout duration; production-like profiles require the Redis-backed store. `X-Forwarded-For` is ignored unless `TRASCK_TRUST_FORWARDED_FOR=true` is set behind a trusted proxy. Login, agent callback, and generic worker authentication failures write durable security telemetry rows.
...
Current external Playwright coverage in `../trasck-test` includes Java Playwright smoke tests for backend health, backend CSRF metadata, setup endpoint reachability, unauthenticated current-user denial, optional login/current-user cookie and Bearer sessions, local OpenAPI availability, and frontend shell navigation with screenshots/traces/console-error capture. That repo is the planned home for full-stack API, browser, security, and cross-system regression coverage.
