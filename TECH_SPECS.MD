...
63. Import jobs support CSV, Jira JSON, and Rally JSON parsing into reviewable import records. Import mapping templates can materialize parsed records into work items through the same service path as normal work item creation/update, with reusable versioned workspace transform presets, first-class immutable transform preset version records, template-local transformation overrides, legacy transformation functions, argument-based transformation pipelines, value lookup rules, and type/status translation rules applied before materialization. Each materialization writes an immutable snapshot row containing the mapping template, mapping rules, transform preset version/config, merged transformation config, counts, status, actor, and target project. Import records have append-only lifecycle version snapshots for parse, edit, materialization, conflict-opened, and conflict-resolved changes. Import conflicts are first-class reviewable record state, `update_existing` conflict resolution stages the record as pending for a later materialization/rerun, transform preset historical versions can be cloned into standalone new presets or cloned and retargeted to selected mapping templates through separate preview/apply APIs, exact rerun from a materialization snapshot reuses the stored template/rules/transform snapshot by default while allowing an explicit admin `updateExisting` override for modified reruns, and import completion may proceed with open conflicts only when the caller explicitly accepts that state.
