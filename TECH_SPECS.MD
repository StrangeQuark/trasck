...
67. A full production-minded security audit has been recorded in `SECURITY_AUDIT.MD`. Production hardening remains required before manual end-to-end product testing, with priority on removing tracked `.env` files, rejecting unsafe production defaults, gating Swagger/OpenAPI, enforcing explicit CORS, adding outbound egress/SSRF controls, tightening login/password policy, and adding security regression tests. Production secrets must be injected through environment variables or local `.env` files that are not committed; checked-in config may contain only safe local examples.
...
- The production-minded backend/frontend security sweep is recorded in `SECURITY_AUDIT.MD`. The audit found no `6101` configuration references and no frontend access-token persistence, but it identified production hardening work that must be completed before broad manual end-to-end testing: untrack real `.env` files and keep only examples, reject dev secrets under production-like profiles, gate Swagger/OpenAPI, implement explicit CORS, add outbound egress/SSRF controls, add login/password protections, enforce upload/import size limits, harden frontend secret display and container headers, and add dependency/security regression gates.
