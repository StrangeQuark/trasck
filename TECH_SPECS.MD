...
65. Manual worker endpoints remain available, and scheduled automation, webhook, email, and import conflict-resolution workers are enabled per workspace through `automation_worker_settings`; defaults keep all scheduled workers off. Worker executions write dedicated run-history and health rows for automation, webhook, and email workers, while import conflict-resolution worker history is visible through the persisted conflict-resolution job rows. Worker run retention settings, manual retention export, manual retention pruning, and optional automatic scheduled pruning are implemented. Manual worker-run pruning bypasses automatic schedule/window/throttle controls; automatic worker-run pruning has a dedicated fixed-delay scheduler, per-workspace interval throttle, optional workspace-timezone execution window, and last-start/last-finish metadata.
...
- `import_jobs`, `import_conflict_resolution_jobs`, `import_transform_presets`, `import_mapping_templates`, import mapping rule tables, `import_materialization_runs`, `import_job_records`, and `import_job_record_versions` provide observable import progress, backend sample job creation, reusable source mappings, reusable transformation pipelines, transformation/translation rules, materialization output, exact rerun snapshots, conflict review, queued large filtered conflict resolution, completion-audit/reporting fields, lifecycle history, normalized record/job diff rows, and error reporting.
...
- Import job tests for idempotency, partial failure handling, record version history and backend diff rows, job-level diff exports/artifacts, filtered record lists, bulk conflict resolution, queued filtered conflict-resolution job cancel/retry/failure/worker behavior, scheduled worker-settings pickup, clone-retarget behavior, guarded completion fields, sample payload and backend sample job flows, and snapshot reruns.
