...
36. AI provider adapters keep non-external stubs as the default, while Codex and Claude Code also expose explicitly enabled `hosted_api` and hardened `cli_worker` runtime modes behind the same provider-neutral adapter contract. CLI runtime config must reference an allowlisted `commandProfile`; Trasck does not accept raw shell commands as provider config.
