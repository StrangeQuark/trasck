...
67. A full production-minded security audit has been recorded in `SECURITY_AUDIT.MD`. The current hardening baseline removes tracked `.env` files, adds committed examples only, rejects unsafe production defaults, gates Swagger/OpenAPI behind dedicated system-admin access in production-like profiles, enforces explicit CORS, adds outbound egress/SSRF controls with exact host, wildcard host, and CIDR allowlisting, tightens login/password policy, persists auth throttling and auth-failure telemetry, rate-limits agent callback/worker auth failures, enforces attachment/import/export size and content-type limits, masks frontend secret displays, templates production nginx CSP, wires Java dependency vulnerability scanning, and adds targeted regression tests. Remaining production hardening should focus on system-admin management UX, Redis-backed throttling for horizontal scale, dependency-scanner cache/API-key strategy, stronger secret-management/KMS abstraction, centralized production error/log redaction, and broader redaction/tenant-isolation regression coverage. Production secrets must be injected through environment variables or local `.env` files that are not committed; checked-in config may contain only safe local examples.
...
- New human passwords are validated for configured minimum length and common development password rejection during setup, invitation registration, and workspace-admin direct user creation. Login failures are throttled by identifier plus remote address with configurable database-backed counters, failure window, and lockout duration; `X-Forwarded-For` is ignored unless `TRASCK_TRUST_FORWARDED_FOR=true` is set behind a trusted proxy. Login, agent callback, and generic worker authentication failures write durable security telemetry rows.
...
- Attachment metadata/upload/download, generated export artifacts/downloads, and import parser payloads enforce configured byte limits and allowlisted content types through shared security policy. Multipart request size limits are also environment-configurable.
