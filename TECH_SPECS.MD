...
Current external Playwright coverage in `../trasck-test` includes Java Playwright smoke tests for backend health, backend CSRF metadata, setup endpoint reachability, unauthenticated current-user denial, optional login/current-user cookie and Bearer sessions, local OpenAPI availability, authenticated API coverage for workspace/project security policy, work item collaboration, teams/planning/boards, dashboards, saved filters, saved views, favorites, and recent items when credentials plus workspace/project IDs are configured, resource cleanup through public APIs, and frontend shell navigation with screenshots/traces/console-error capture. That repo is the planned home for full-stack API, browser, security, and cross-system regression coverage.
