...
63. Import jobs support CSV, Jira JSON, and Rally JSON parsing into reviewable import records. Import mapping templates can materialize parsed records into work items through the same service path as normal work item creation/update, with reusable versioned workspace transform presets, first-class immutable transform preset version records, template-local transformation overrides, legacy transformation functions, argument-based transformation pipelines, value lookup rules, and type/status translation rules applied before materialization. Each materialization writes an immutable snapshot row containing the mapping template, mapping rules, transform preset version/config, merged transformation config, counts, status, actor, and target project. Import records can be edited before materialization. Import conflicts are first-class reviewable record state, `update_existing` conflict resolution stages the record as pending for a later materialization/rerun, transform preset historical versions can be cloned into standalone new presets, and exact rerun from a materialization snapshot reuses the stored template/rules/transform snapshot by default while allowing an explicit admin `updateExisting` override for modified reruns.
...
- `GET /api/v1/workspaces/{workspaceId}/import-jobs`, `POST /api/v1/workspaces/{workspaceId}/import-jobs`, `GET /api/v1/import-jobs/{importJobId}`, `POST /api/v1/import-jobs/{importJobId}/parse`, `POST /api/v1/import-jobs/{importJobId}/materialize`, `GET /api/v1/import-jobs/{importJobId}/materialization-runs`, `POST /api/v1/import-materialization-runs/{materializationRunId}/rerun`, `GET /api/v1/import-jobs/{importJobId}/conflicts`, `POST /api/v1/import-job-records/{recordId}/resolve-conflict`, `POST /api/v1/import-jobs/{importJobId}/start`, `POST /api/v1/import-jobs/{importJobId}/complete`, `POST /api/v1/import-jobs/{importJobId}/fail`, `POST /api/v1/import-jobs/{importJobId}/cancel`, `GET /api/v1/import-jobs/{importJobId}/records`, `POST /api/v1/import-jobs/{importJobId}/records`, `PATCH /api/v1/import-job-records/{recordId}`, `GET /api/v1/workspaces/{workspaceId}/import-transform-presets`, `POST /api/v1/workspaces/{workspaceId}/import-transform-presets`, `GET /api/v1/import-transform-presets/{presetId}`, `PATCH /api/v1/import-transform-presets/{presetId}`, `DELETE /api/v1/import-transform-presets/{presetId}`, `GET /api/v1/import-transform-presets/{presetId}/versions`, `POST /api/v1/import-transform-presets/{presetId}/versions/{versionId}/clone`, `GET /api/v1/workspaces/{workspaceId}/import-mapping-templates`, `POST /api/v1/workspaces/{workspaceId}/import-mapping-templates`, `PATCH /api/v1/import-mapping-templates/{mappingTemplateId}`, `DELETE /api/v1/import-mapping-templates/{mappingTemplateId}`, and mapping-template child endpoints for value lookups, type translations, and status translations manage import lifecycle, CSV/Jira/Rally parsing, per-source records, reusable versioned transform presets, immutable preset version history, clone-from-version, mapping templates, mapping rules, materialization snapshots, conflict review/resolution, exact and modified snapshot reruns, and work item materialization.
...
- Import job list/detail/lifecycle/parse/record/template/preset/version/materialization/conflict/rerun APIs require workspace `workspace.admin`. CSV, Jira JSON, and Rally JSON parsers create reviewable import records; admins can edit imported records before materialization, and those edits clear stale conflict state. Mapping templates can create or update work items through the existing work item service after applying transform presets, template-local transformation config, value lookups, type translations, and status translations, preserving project permissions, sequence allocation, workflow defaults, screen/custom-field validation, history writes, and domain events. Conflict resolution can mark a record skipped, stage it for update-existing processing, or clear the target so a rerun can create a new work item.
...
- `import_jobs`, `import_transform_presets`, `import_mapping_templates`, import mapping rule tables, `import_materialization_runs`, and `import_job_records` provide observable import progress, reusable source mappings, reusable transformation pipelines, transformation/translation rules, materialization output, exact rerun snapshots, conflict review, and error reporting.
...
- Import job tests for idempotency, partial failure handling, conflict resolution, and snapshot reruns.
