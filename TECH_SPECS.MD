...
67. A full production-minded security audit has been recorded in `SECURITY_AUDIT.MD`. The first hardening pass removed tracked `.env` files, added committed examples only, rejects unsafe production defaults, gates Swagger/OpenAPI in production-like profiles, enforces explicit CORS, adds outbound egress/SSRF controls, tightens login/password policy, and adds targeted regression tests. Remaining production hardening should prioritize upload/import size controls, frontend secret-display polish, Java dependency vulnerability automation, callback/worker auth-failure rate/audit controls, broader redaction tests, and the production OpenAPI/system-admin decision. Production secrets must be injected through environment variables or local `.env` files that are not committed; checked-in config may contain only safe local examples.
...
- The production-minded backend/frontend security sweep is recorded in `SECURITY_AUDIT.MD`. The first hardening pass removed checked-in `.env` files in favor of `.env.example`, rejects unsafe production-like startup defaults, gates Swagger/OpenAPI behind authenticated admin access in production-like profiles, enables explicit CORS, adds default-deny outbound URL/SSRF policy and HTTP timeouts for configured outbound calls, tightens password/login protections, updates frontend container builds to `npm ci`, and adds baseline nginx security headers. Remaining production work includes upload/import size and content-type limits, frontend secret-display polish, Java dependency vulnerability automation, callback/worker auth-failure rate/audit controls, stronger secret-management/KMS abstraction, centralized production error/log redaction, and a broader dedicated security regression suite.
- Setup, auth login/register/OAuth/login/logout, health, and public project reads are explicitly permitted.
- OpenAPI docs and Swagger UI are read-only and permitted without authentication for local/non-production contract discovery. Production-like `prod`, `production`, `staging`, and `hosted` profiles require an authenticated principal with `workspace.admin` scope and an active workspace admin role.
...
- New human passwords are validated for configured minimum length and common development password rejection during setup, invitation registration, and workspace-admin direct user creation. Login failures are throttled by identifier plus remote address with configurable in-memory window and lockout duration; `X-Forwarded-For` is ignored unless `TRASCK_TRUST_FORWARDED_FOR=true` is set behind a trusted proxy.
- Configured outbound HTTP targets for webhooks, agent worker callbacks, OAuth provider verified-email side fetches, and S3-compatible storage are checked through a shared outbound URL policy that default-denies localhost, private networks, link-local ranges, carrier-grade NAT, unique-local IPv6, multicast, and metadata-like local destinations unless the exact host or host:port is explicitly allowlisted.
