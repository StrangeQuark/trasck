...
54. OpenAPI is generated by SpringDoc and exposed at `/v3/api-docs` with Swagger UI at `/swagger-ui.html`; when frontend implementation starts, the generated TypeScript client should be checked into the Vite React repo and regenerated only when the backend contract changes.
...
56. The first custom-field search pass supports exact scalar matching for searchable custom fields on project work-item lists. Rich typed operators, ranges, and array matching are reserved for the saved-filter/query-builder pass.
57. Saved views, favorites, and recent items share a single personalization API package. Saved views currently support private, workspace, and public visibility; project/team-scoped saved-view sharing is deferred until the frontend view model needs it.
...
- `GET /api/v1/projects/{projectId}/work-items` lists active project work items in rank order through the cursor-page envelope. Optional `customFieldKey` plus `customFieldValue` filters by exact scalar value for searchable custom fields that apply to the project.
...
- `POST /api/v1/work-items/{workItemId}/team` changes explicit team assignment and writes team history.
...
- `GET /api/v1/screens/{screenId}/assignments`, `POST /api/v1/screens/{screenId}/assignments`, `PATCH /api/v1/screens/{screenId}/assignments/{assignmentId}`, and `DELETE /api/v1/screens/{screenId}/assignments/{assignmentId}` manage screen selection metadata by project, work item type, and create/edit/view operation.
...
- `GET /api/v1/workspaces/{workspaceId}/saved-filters`, `GET /api/v1/projects/{projectId}/saved-filters`, `GET /api/v1/teams/{teamId}/saved-filters`, `POST /api/v1/workspaces/{workspaceId}/saved-filters`, `GET /api/v1/saved-filters/{savedFilterId}`, `PATCH /api/v1/saved-filters/{savedFilterId}`, and `DELETE /api/v1/saved-filters/{savedFilterId}` manage saved filters with dashboard-style visibility and optional project/team query scoping.
...
- Custom field and screen definition reads require workspace `workspace.read`; definition/context/screen/assignment mutations require workspace `workspace.admin`. Work item custom field value reads use project `work_item.read`, and value writes/deletes use project `work_item.update`.
- Saved view reads require workspace `workspace.read` and visibility ownership. Private saved view owners can edit/delete their own views with workspace read access; shared workspace/public saved-view writes require workspace `workspace.admin`. Favorites and recent items are per-user and require workspace read access plus a target entity that belongs to that workspace.
...
- Work item create/update can set or clear an explicit team assignment through `work_items.team_id`. The dedicated `POST /api/v1/work-items/{workItemId}/team` command is the preferred mutation path and writes `work_item_team_history` plus `work_item.team_changed`. The team must be active, belong to the same workspace, and be assigned to the work item's project through `project_teams`.
- Team and planning APIs expose the core configuration that reporting and work item assignment depend on. Workspace admins can create/update/archive teams and manage immediate membership joins/leaves. Project admins can assign active teams to projects and update the current project-team role. Board admins can create/update/cancel project-scoped iterations, add/remove iteration work items, commit current scope points, close iterations, and carry incomplete work into another compatible project/team iteration.
...
- Work item custom field values are stored in `custom_field_values` after custom field context/type validation. Project work-item lists can filter by `customFieldKey` and exact scalar `customFieldValue` when the field is marked searchable and applies to the project. A scalar expression index supports that first-pass filter shape.
...
- Saved dashboards support private, team-shared, project-shared, workspace-shared, and public visibility. Dashboards, saved filters, and report query catalog entries have direct workspace/project/team list endpoints so scoped UI screens can use navigation IDs directly. Saved views, favorites, and recent items share the personalization API; saved views currently support private/workspace/public visibility.
...
8. Implement boards, columns, iterations, releases, and roadmaps. Project-scoped iteration CRUD, scope add/remove, commit, close, and carryover APIs are implemented; board, release, and roadmap APIs remain.
9. Implement custom fields, screens, screen assignments, and field configurations. Custom field definition/context/value APIs, first-pass exact searchable filtering, screen metadata/field/assignment APIs, and integration coverage are implemented; field configuration CRUD and screen-driven work-item form enforcement remain.
