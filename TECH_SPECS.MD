...
52. Reporting snapshot retention should favor long-lived dashboards: design daily-to-weekly/monthly rollups and archive metadata before enforcing destructive snapshot pruning.
...
- `GET /api/v1/workspaces/{workspaceId}/report-query-catalog`, `POST /api/v1/workspaces/{workspaceId}/report-query-catalog`, `GET /api/v1/report-query-catalog/{queryId}`, `PATCH /api/v1/report-query-catalog/{queryId}`, and `DELETE /api/v1/report-query-catalog/{queryId}` manage governed reusable reporting queries for dashboard widgets. `parametersSchema` supports `string`, `uuid`, `integer`, `number`, `boolean`, `date`, `datetime`, `array`, and `object` runtime validation.
...
- For long-lived dashboards, design rollups before destructive retention: keep raw daily rows for the short window, materialize weekly/monthly aggregates for older windows, and retain archive metadata so dashboard series stay stable after raw rows age out.
