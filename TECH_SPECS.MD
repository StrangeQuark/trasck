...
19. Anonymous read access for public project and public work-item data must be wired into the first security configuration, but every public read must still pass domain-level visibility checks.
...
36. AI provider adapters keep non-external stubs as the default, while Codex and Claude Code also expose explicitly enabled `hosted_api` and hardened `cli_worker` runtime modes behind the same provider-neutral adapter contract. Backend-managed CLI execution is additionally disabled by environment by default and must be enabled on a trusted worker host. CLI runtime config must reference an allowlisted `commandProfile`; Trasck does not accept raw shell commands as provider config.
...
54. OpenAPI is generated by SpringDoc and exposed at `/v3/api-docs` with Swagger UI at `/swagger-ui.html`; the Vite React repo has checked-in generation tooling and committed generated OpenAPI/TypeScript artifacts that should be regenerated immediately whenever backend routes or schemas change and a backend is available on the local contract port.
...
59. Frontend components should use feature-specific API services and hooks around the generated `TrasckApiClient`; the generated TypeScript client and `openapi.json` stay committed as the reviewed contract snapshot while the API is moving quickly. Browser cookie-authenticated unsafe requests use Spring cookie CSRF and send `X-XSRF-TOKEN`, and browser clients must fetch a fresh CSRF token for each unsafe request instead of caching one across setup, login, logout, or other session-rotating flows. The Vite frontend uses route-based navigation rather than a single tabbed console shell, the API base URL comes only from environment/runtime configuration instead of a browser-visible editor, authenticated workspace/project context comes from `/api/v1/auth/context`, that context includes effective workspace/project `permissionKeys` plus `systemAdmin` state for permission-aware tabs/routes/page loads, and the Project Settings route is the long-term home for project-admin settings as additional project-scoped configuration APIs are added. Product pages should present structured tables, summaries, forms, and selectors; raw payload diagnostics are gated behind `VITE_TRASCK_ENABLE_DIAGNOSTICS=true` and are not normal UI.
...
63. Import jobs support CSV, Jira JSON, and Rally JSON parsing into reviewable import records. Import mapping templates can materialize parsed records into work items through the same service path as normal work item creation/update, with reusable versioned workspace transform presets, first-class immutable transform preset version records, template-local transformation overrides, legacy transformation functions, argument-based transformation pipelines, value lookup rules, and type/status translation rules applied before materialization. Each materialization writes an immutable snapshot row containing the mapping template, mapping rules, transform preset version/config, merged transformation config, counts, status, actor, and target project. Import records have append-only lifecycle version snapshots for parse, edit, materialization, conflict-opened, and conflict-resolved changes, and the backend exposes safe empty record lists for jobs with no records, normalized adjacent field-level diff rows plus job-level diff/export groupings for API clients, stored JSON/CSV export-job audit artifacts, and frontend review tables with column filtering. Job-level diff CSV exports, conflict-resolution job audit table exports, import export-artifact table exports, and project/workspace import-completion metric exports are backend-generated export jobs that reuse the current table filter state; smaller local review tables may still use client-side CSV until a backend export is needed. Import conflicts are first-class reviewable record state, `update_existing` conflict resolution stages the record as pending for a later materialization/rerun, selected bulk conflict resolution stays explicit by record ID, filtered bulk conflict resolution requires a paginated preview count plus exact `RESOLVE FILTERED CONFLICTS` confirmation and matching `expectedCount`, synchronous filtered apply is capped to a guarded batch size, larger filtered sets can be queued as persisted import conflict-resolution jobs, queued jobs can be listed by job or workspace, canceled, retried, manually run, processed by a bounded worker endpoint, or picked up by the shared scheduled worker through workspace `automation_worker_settings`, transform preset historical versions can be cloned into standalone new presets or cloned and retargeted to selected mapping templates through separate preview/apply APIs, exact rerun from a materialization snapshot reuses the stored template/rules/transform snapshot by default while allowing an explicit admin `updateExisting` override for modified reruns, import completion may proceed with open conflicts only when the caller sends the typed `COMPLETE WITH OPEN CONFLICTS` confirmation and an audit reason that is persisted on the import job for reporting/dashboard summaries and dedicated import-completion report/widget endpoints, and workspace admins can create backend-generated CSV/Jira/Rally sample jobs from the import sample catalog for guided manual testing. Sample job endpoints are workspace-admin-only, controlled by `import_workspace_settings.sample_jobs_enabled`, and production-like deployments with `prod`, `production`, `staging`, or `hosted` profiles must also explicitly set `TRASCK_IMPORT_SAMPLE_JOBS_ENABLED=true` before sample job endpoints are available.
64. Boards expose per-column work item card data from configured status mappings plus configured backend swimlane groupings backed by the saved-filter/query-builder predicate engine. Swimlanes may reference saved filter IDs directly or store inline query JSON. Board-scoped rank, transition, and combined move commands validate board membership before using the normal work item command paths; transition/move commands can use explicit `transitionKey` or derive an allowed workflow transition from `targetColumnId` and/or `targetStatusId`. Drag/drop uses card-level insertion by sending `previousWorkItemId`/`nextWorkItemId` relative to the target column. This board engine is the planning foundation. The first product-facing planning slice now exposes distinct Overview, Backlog, Active Board, and Admin routes; board queries accept optional `iterationId`, `teamId`, and `viewMode` filters so the same board definition can back active-sprint and backlog views; and board column configuration is driven by selectable workflow-status options rather than raw JSON status editing. Further planning work should focus on iteration close/carryover ergonomics and richer sprint-health context rather than basic route separation.
65. Manual worker endpoints remain available, and scheduled automation, webhook, email, import conflict-resolution, and import review CSV export workers are enabled per workspace through `automation_worker_settings`; defaults keep all scheduled workers off. Worker executions write dedicated run-history and health rows for automation, webhook, email, import conflict-resolution, and import review export workers. Worker run/health APIs accept an optional `workerType` filter for operations screens. Worker run retention settings, manual retention export, manual retention pruning, optional `workerType` filters on manual export/prune, and optional automatic scheduled pruning are implemented. Manual worker-run pruning bypasses automatic schedule/window/throttle controls; automatic worker-run pruning has a dedicated fixed-delay scheduler, per-workspace interval throttle, optional workspace-timezone execution window, and last-start/last-finish metadata.
...
67. A full production-minded security audit has been recorded in `SECURITY_AUDIT.MD`. The current hardening baseline removes tracked `.env` files, adds committed examples only, rejects unsafe production defaults, requires Redis-backed auth throttling in production-like profiles, pings Redis during production-like startup, gates Swagger/OpenAPI behind dedicated system-admin access in production-like profiles, exposes system-admin grant/revoke/list management with production-like recent-login step-up on grant/revoke, enforces explicit CORS, adds outbound egress/SSRF controls with exact host, wildcard host, and CIDR allowlisting, tightens login/password policy, persists auth-failure telemetry, rate-limits agent callback/worker auth failures, enforces workspace-admin and project-admin configurable attachment/import/export size and content-type policy, masks frontend secret displays, centralizes redacted API error bodies/log messages while preserving explicit client-error status shapes, templates production nginx CSP, wires Java dependency vulnerability scanning as a required CI gate, and adds targeted regression tests. Remaining production hardening should focus on stronger secret-management/KMS abstraction, production health/observability for Redis-backed throttling, step-up UX hardening, and broader redaction/tenant-isolation regression coverage. Production secrets must be injected through environment variables or local `.env` files that are not committed; checked-in config may contain only safe local examples.
...
- `Program` groups projects for portfolio reporting and planning and stores roadmap/report configuration JSON for portfolio views.
...
- `GET /api/v1/setup/status` returns whether first-run setup is still available and whether it has already completed. It is public, read-only, and exists so browser clients can hide setup affordances after bootstrap.
- `POST /api/v1/setup` creates only the initial super-admin user and system-admin grant. It is first-run-only and returns conflict after bootstrap; organizations, workspaces, projects, teams, and normal product records are created through authenticated product APIs after login.
- `GET /api/v1/organizations`, `POST /api/v1/organizations`, `GET /api/v1/organizations/{organizationId}`, `PATCH /api/v1/organizations/{organizationId}`, and `DELETE /api/v1/organizations/{organizationId}` provide authenticated organization management. System admins and organization creators can manage organizations; active workspace members can read their organizations, and workspace admins can help manage the organization container.
- `GET /api/v1/organizations/{organizationId}/workspaces`, `POST /api/v1/organizations/{organizationId}/workspaces`, `GET /api/v1/workspaces/{workspaceId}`, `PATCH /api/v1/workspaces/{workspaceId}`, and `DELETE /api/v1/workspaces/{workspaceId}` provide authenticated workspace management. Workspace creation seeds deterministic workspace defaults, attachment storage config, default roles, and an owner membership for the creator.
- `GET /api/v1/workspaces/{workspaceId}/projects`, `POST /api/v1/workspaces/{workspaceId}/projects`, `GET /api/v1/projects/{projectId}`, `PATCH /api/v1/projects/{projectId}`, and `DELETE /api/v1/projects/{projectId}` provide authenticated project management. Project creation uses the seeded workspace defaults to create project work-item type enablement, workflow assignments, the default board, project settings, and the creator's project-admin membership.
...
- `GET /api/v1/auth/context` returns the authenticated user plus active workspace memberships, active project memberships, effective workspace/project `permissionKeys`, `systemAdmin` state, and default workspace/project selections for the product shell.
...
- `GET /api/v1/workspaces/{workspaceId}/invitations` lists workspace invitations for callers with `user.manage`, defaults to `status=pending`, supports `status=all`, and never returns raw invitation tokens or token hashes.
- `POST /api/v1/workspaces/{workspaceId}/invitations` creates a workspace invitation for callers with `user.manage`.
- `DELETE /api/v1/workspaces/{workspaceId}/invitations/{invitationId}` revokes a pending workspace invitation for callers with `user.manage`; accepted invitations cannot be revoked.
...
- `GET /api/v1/workspaces/{workspaceId}/users` lists human workspace memberships plus safe user profile fields for callers with `user.manage`, defaults to `status=active`, supports `status=all`, and never returns password hashes.
- `POST /api/v1/workspaces/{workspaceId}/users` lets callers with `user.manage` create a user directly in a workspace.
- `DELETE /api/v1/workspaces/{workspaceId}/users/{userId}` removes an active user from a workspace, removes that user's active project memberships inside the workspace, and deactivates the user if no other active workspace memberships remain.
- Workspace/project role management APIs list permission catalogs, list/detail/create/update/archive roles, preview permission impact, confirm permission changes, list immutable role versions, and rollback roles through `GET/POST/PATCH/DELETE/PUT /api/v1/workspaces/{workspaceId}/roles...` and equivalent `GET/POST/PATCH/DELETE/PUT /api/v1/projects/{projectId}/roles...` routes. Workspace role management requires `user.manage`; project role management requires project `project.admin`; permission saves require a fresh preview token and preserve at least one workspace/project admin path.
- `GET /api/v1/system-admins`, `POST /api/v1/system-admins`, and `DELETE /api/v1/system-admins/{userId}` let active system admins list, grant, and revoke dedicated system-admin access while preserving at least one active system admin. Production-like profiles require the acting system admin to have authenticated recently before grant/revoke.
- `GET /api/v1/workspaces/{workspaceId}/security-policy` and `PATCH /api/v1/workspaces/{workspaceId}/security-policy` let workspace admins inspect and update effective attachment, import, and export size/content-type policies plus the workspace anonymous-read switch.
- `GET /api/v1/projects/{projectId}/security-policy` and `PATCH /api/v1/projects/{projectId}/security-policy` let project admins inspect inherited workspace limits, apply project-level attachment/import/export size/content-type overrides, and update project visibility. Responses include `workspaceAnonymousReadEnabled` and effective `publicReadEnabled`.
...
- `GET /api/v1/public/projects/{projectId}/work-items` returns a cursor page of non-private work items for an anonymously readable public project.
- `GET /api/v1/public/projects/{projectId}/work-items/{workItemId}` returns one non-private work item for an anonymously readable public project. Public work-item responses intentionally use a narrower DTO than authenticated work item reads and omit user IDs such as reporter and assignee.
- `GET /api/v1/public/projects/{projectId}/work-items/{workItemId}/comments` returns non-deleted, non-private comments for a public work item. Public comment responses omit author IDs, updater IDs, and edit metadata; comments inherit work-item public visibility unless explicitly private, and become unavailable when the work item becomes private.
- `GET /api/v1/public/projects/{projectId}/work-items/{workItemId}/attachments` returns public attachment metadata for a public work item. Public attachment responses omit storage configuration IDs, storage keys, and uploader IDs, and include a short-lived app-signed backend `downloadUrl`. App-signed backend proxy URLs are the long-term public attachment download pattern for both local and S3-compatible storage.
- `GET /api/v1/public/projects/{projectId}/work-items/{workItemId}/attachments/{attachmentId}/download?token=...` streams a public attachment only after the signed URL token, workspace/project public-read state, work-item visibility, attachment visibility, and effective download policy all pass.
- `GET /api/v1/workspaces/{workspaceId}/programs`, `POST /api/v1/workspaces/{workspaceId}/programs`, `GET /api/v1/programs/{programId}`, `PATCH /api/v1/programs/{programId}`, and `DELETE /api/v1/programs/{programId}` manage workspace programs with `active`/`archived` status plus JSON-object `roadmapConfig` and `reportConfig`.
- `GET /api/v1/programs/{programId}/projects`, `PUT /api/v1/programs/{programId}/projects/{projectId}`, and `DELETE /api/v1/programs/{programId}/projects/{projectId}` manage active project assignments inside active programs.
...
- `GET /api/v1/boards/{boardId}/status-options` returns selectable workflow status options for board-column configuration UIs based on workflows assigned to the board's project.
- `GET /api/v1/boards/{boardId}/work-items` returns per-column work item cards using configured column status mappings and a bounded `limitPerColumn`, plus configured swimlane groupings over those columns. Optional `iterationId`, `teamId`, and `viewMode=all|iteration|backlog` filters let the same board definition power active-sprint and backlog-style planning views. Swimlanes can either reference a readable saved filter ID or store inline query JSON that reuses the saved-filter/query-builder predicate shape.
- `POST /api/v1/boards/{boardId}/work-items/{workItemId}/rank`, `POST /api/v1/boards/{boardId}/work-items/{workItemId}/transition`, and `POST /api/v1/boards/{boardId}/work-items/{workItemId}/move` provide board-scoped rank, transition, and combined move commands that validate the work item belongs to the board project before delegating to the normal work item command paths. Transition/move requests can send `transitionKey` directly, or send `targetColumnId` and/or `targetStatusId` so drag/drop flows can derive the allowed workflow transition from board column/status mappings. Move requests may also include `previousWorkItemId` and/or `nextWorkItemId` so status transition and rank update happen in one backend transaction.
- `GET /api/v1/boards/{boardId}/swimlanes`, `POST /api/v1/boards/{boardId}/swimlanes`, `PATCH /api/v1/boards/{boardId}/swimlanes/{swimlaneId}`, and `DELETE /api/v1/boards/{boardId}/swimlanes/{swimlaneId}` manage board swimlane configuration, including optional `savedFilterId` references and inline query JSON.
...
- `GET /api/v1/reports/projects/{projectId}/imports/completions` and `GET /api/v1/reports/workspaces/{workspaceId}/imports/completions` expose focused import completion/open-conflict acceptance metrics for dashboard widgets and import audit screens.
...
- `GET /api/v1/workspaces/{workspaceId}/notifications`, `POST /api/v1/workspaces/{workspaceId}/notifications`, `PATCH /api/v1/notifications/{notificationId}/read`, `GET /api/v1/workspaces/{workspaceId}/notification-preferences`, `POST /api/v1/workspaces/{workspaceId}/notification-preferences`, `PATCH /api/v1/notification-preferences/{preferenceId}`, and `DELETE /api/v1/notification-preferences/{preferenceId}` expose current-user notifications, workspace-admin direct notification creation, notification read state, and notification preferences.
...
- `GET /api/v1/workspaces/{workspaceId}/automation-rules`, `POST /api/v1/workspaces/{workspaceId}/automation-rules`, `GET /api/v1/automation-rules/{ruleId}`, `PATCH /api/v1/automation-rules/{ruleId}`, `DELETE /api/v1/automation-rules/{ruleId}`, condition/action child routes, `POST /api/v1/automation-rules/{ruleId}/execute`, `GET /api/v1/automation-rules/{ruleId}/jobs`, `GET /api/v1/automation-jobs/{jobId}`, `POST /api/v1/automation-jobs/{jobId}/run`, `POST /api/v1/workspaces/{workspaceId}/automation-jobs/run-queued`, `GET /api/v1/workspaces/{workspaceId}/automation-worker-settings`, `PATCH /api/v1/workspaces/{workspaceId}/automation-worker-settings`, `GET /api/v1/workspaces/{workspaceId}/automation-worker-runs?workerType=...`, `POST /api/v1/workspaces/{workspaceId}/automation-worker-runs/export?workerType=...&triggerType=...&status=...&startedFrom=...&startedTo=...`, `POST /api/v1/workspaces/{workspaceId}/automation-worker-runs/prune?workerType=...&triggerType=...&status=...&startedFrom=...&startedTo=...`, and `GET /api/v1/workspaces/{workspaceId}/automation-worker-health?workerType=...` manage automation rules, queued worker execution, workspace-level scheduled worker settings, worker run history, worker-type filtered operations views, manual worker-run retention export/prune with optional worker type, trigger type, status, and start-time range filters, optional automatic worker-run pruning with interval/window/last-run metadata, and worker health.
- `GET /api/v1/workspaces/{workspaceId}/webhooks`, `POST /api/v1/workspaces/{workspaceId}/webhooks`, `PATCH /api/v1/webhooks/{webhookId}`, `DELETE /api/v1/webhooks/{webhookId}`, `GET /api/v1/webhooks/{webhookId}/deliveries`, `GET /api/v1/webhook-deliveries/{deliveryId}`, `POST /api/v1/webhook-deliveries/{deliveryId}/retry`, `POST /api/v1/webhook-deliveries/{deliveryId}/cancel`, and `POST /api/v1/workspaces/{workspaceId}/webhook-deliveries/process` manage webhook configuration, queued delivery records, inspection, retry/cancel administration, dry-run or real HTTP processing, and dead-letter state. Webhook secrets are hashed for redacted configured-state checks and encrypted for outbound delivery signing. Secret writes rotate a non-secret `secretKeyId`, retain the immediately previous encrypted secret/key/hash for a per-webhook configurable positive overlap duration stored as `previousSecretOverlapSeconds`, default new webhooks from the 30-day deployment default `trasck.webhooks.previous-secret-overlap` / `TRASCK_WEBHOOK_PREVIOUS_SECRET_OVERLAP`, and expose previous-key expiry metadata. Queued deliveries persist the selected `signatureKeyId`, so deliveries queued before rotation can still be signed with the previous secret during overlap. Receiver verification/rotation guidance lives in `docs/WEBHOOK_SIGNATURES.md`. Real deliveries include `X-Trasck-Webhook-Id`, `X-Trasck-Webhook-Delivery-Id`, `X-Trasck-Webhook-Timestamp`, `X-Trasck-Webhook-Signature-Key-Id`, and `X-Trasck-Webhook-Signature` headers; the signature is `sha256=` plus an HMAC-SHA256 over `timestamp + "." + body`.
...
- `GET /api/v1/workspaces/{workspaceId}/import-jobs`, `POST /api/v1/workspaces/{workspaceId}/import-jobs`, `GET/PATCH /api/v1/workspaces/{workspaceId}/import-settings`, `GET /api/v1/workspaces/{workspaceId}/import-samples`, `POST /api/v1/workspaces/{workspaceId}/import-samples/{sampleKey}/jobs`, `GET /api/v1/import-jobs/{importJobId}`, `POST /api/v1/import-jobs/{importJobId}/parse`, `POST /api/v1/import-jobs/{importJobId}/materialize`, `GET /api/v1/import-jobs/{importJobId}/materialization-runs`, `POST /api/v1/import-materialization-runs/{materializationRunId}/rerun`, `GET /api/v1/import-jobs/{importJobId}/conflicts`, `POST /api/v1/import-job-records/{recordId}/resolve-conflict`, `POST /api/v1/import-jobs/{importJobId}/conflicts/resolve-preview`, `POST /api/v1/import-jobs/{importJobId}/conflicts/resolve`, `POST /api/v1/import-jobs/{importJobId}/conflicts/resolve-async`, `GET /api/v1/import-jobs/{importJobId}/conflict-resolution-jobs`, `GET /api/v1/workspaces/{workspaceId}/import-conflict-resolution-jobs`, `GET /api/v1/import-conflict-resolution-jobs/{jobId}`, `POST /api/v1/import-conflict-resolution-jobs/{jobId}/run`, `POST /api/v1/import-conflict-resolution-jobs/{jobId}/cancel`, `POST /api/v1/import-conflict-resolution-jobs/{jobId}/retry`, `POST /api/v1/workspaces/{workspaceId}/import-conflict-resolution-jobs/process`, `POST /api/v1/import-jobs/{importJobId}/start`, `POST /api/v1/import-jobs/{importJobId}/complete`, `POST /api/v1/import-jobs/{importJobId}/fail`, `POST /api/v1/import-jobs/{importJobId}/cancel`, `GET /api/v1/import-jobs/{importJobId}/records`, `POST /api/v1/import-jobs/{importJobId}/records`, `PATCH /api/v1/import-job-records/{recordId}`, `GET /api/v1/import-job-records/{recordId}/versions`, `GET /api/v1/import-job-records/{recordId}/version-diffs`, `GET /api/v1/import-jobs/{importJobId}/version-diffs`, `GET /api/v1/import-jobs/{importJobId}/version-diffs/export`, `POST /api/v1/import-jobs/{importJobId}/version-diffs/export-jobs`, `POST /api/v1/workspaces/{workspaceId}/import-review/export-jobs`, `GET /api/v1/workspaces/{workspaceId}/import-transform-presets`, `POST /api/v1/workspaces/{workspaceId}/import-transform-presets`, `GET /api/v1/import-transform-presets/{presetId}`, `PATCH /api/v1/import-transform-presets/{presetId}`, `DELETE /api/v1/import-transform-presets/{presetId}`, `GET /api/v1/import-transform-presets/{presetId}/versions`, `POST /api/v1/import-transform-presets/{presetId}/versions/{versionId}/clone`, `POST /api/v1/import-transform-presets/{presetId}/versions/{versionId}/retarget-preview`, `POST /api/v1/import-transform-presets/{presetId}/versions/{versionId}/retarget`, `GET /api/v1/workspaces/{workspaceId}/import-mapping-templates`, `POST /api/v1/workspaces/{workspaceId}/import-mapping-templates`, `PATCH /api/v1/import-mapping-templates/{mappingTemplateId}`, `DELETE /api/v1/import-mapping-templates/{mappingTemplateId}`, and mapping-template child endpoints for value lookups, type translations, and status translations manage import lifecycle, CSV/Jira/Rally parsing, workspace-controlled backend-generated sample jobs, filtered per-source records, record version history, normalized record/job diff rows, stored JSON/CSV diff and review-table export artifacts, reusable versioned transform presets, immutable preset version history, clone-from-version, clone-and-retarget preview/apply, mapping templates, mapping rules, materialization snapshots, single/selected/filtered conflict review/resolution, queued filtered conflict-resolution job cancel/retry/worker execution, exact and modified snapshot reruns, guarded completion with typed open-conflict confirmation and first-class audit/report fields, and work item materialization.
- `GET /v3/api-docs` exposes generated OpenAPI JSON and `/swagger-ui.html` exposes Swagger UI for local/backend contract inspection. Production-like profiles require dedicated system-admin access.
...
- The production-minded backend/frontend security sweep is recorded in `SECURITY_AUDIT.MD`. The current hardening baseline removes checked-in `.env` files in favor of `.env.example`, rejects unsafe production-like startup defaults, requires Redis-backed auth throttling in production-like profiles, pings Redis during production-like startup, gates Swagger/OpenAPI behind dedicated system-admin access in production-like profiles, adds dedicated system-admin management APIs and frontend controls with production-like recent-login step-up for grant/revoke, enables explicit CORS, adds default-deny outbound URL/SSRF policy with HTTP timeouts and exact host, wildcard host, and CIDR allowlisting, tightens password/login protections, persists auth-failure telemetry, rate-limits agent callback/worker auth failures, enforces workspace/project configurable attachment/import/export size and content-type limits, masks frontend secret displays, centralizes redacted production API errors/log messages, templates production nginx CSP, wires Java dependency vulnerability scanning as a required CI gate, updates frontend container builds to `npm ci`, and adds baseline nginx security headers. Remaining production work includes stronger secret-management/KMS abstraction, production health/observability for Redis-backed throttling, system-admin step-up UX hardening, and a broader dedicated security regression suite.
- Setup status, first-run setup, auth login/register/OAuth/login/logout, health, and public project/work-item/comment/attachment reads are explicitly permitted.
- OpenAPI docs and Swagger UI are read-only and permitted without authentication for local/non-production contract discovery. Production-like `prod`, `production`, `staging`, and `hosted` profiles require an authenticated principal with `system.admin` scope and an active `system_admins` row.
...
- Browser clients using the auth cookie must send a CSRF token for unsafe methods. Browser clients must fetch that token fresh for each unsafe request because setup/login/logout and other session changes invalidate previously issued CSRF tokens. Bearer-token API callers are not forced through CSRF.
- CORS is explicit and allowlist-based through `CORS_ALLOWED_ORIGINS`; credentialed browser requests from unlisted origins are denied.
- Production-like profiles fail startup if known development JWT/encryption/OAuth assertion secrets, weak database passwords, insecure auth-cookie flags, local OAuth success redirects, or local wildcard CORS origins are active.
- New human passwords are validated for configured minimum length and common development password rejection during setup, invitation registration, and workspace-admin direct user creation. Login failures are throttled by identifier plus remote address with configurable database-backed or Redis-backed counters, failure window, and lockout duration; production-like profiles require the Redis-backed store and fail startup if Redis cannot be pinged. `X-Forwarded-For` is ignored unless `TRASCK_TRUST_FORWARDED_FOR=true` is set behind a trusted proxy. Login, agent callback, and generic worker authentication failures write durable security telemetry rows.
- Configured outbound HTTP targets for webhooks, agent worker callbacks, OAuth provider verified-email side fetches, and S3-compatible storage are checked through a shared outbound URL policy that default-denies localhost, private networks, link-local ranges, carrier-grade NAT, unique-local IPv6, multicast, and metadata-like local destinations unless an exact host, host:port, wildcard host, wildcard host:port, or CIDR allowlist entry explicitly permits the target.
...
- Invitations can grant workspace membership only, or workspace membership plus project-specific membership when `projectId` and `projectRoleId` are included. Pending invitations can be revoked by callers with `user.manage`; accepted invitations are immutable history.
- Workspace-admin direct user cleanup removes the active workspace membership, removes active project memberships in that workspace, prevents self-removal through the cleanup endpoint, and deactivates a user only when they have no remaining active workspace memberships.
- Anonymous public project/work-item/comment/attachment reads still perform domain-level visibility checks; security configuration alone is not the access-control source of truth. Public attachment downloads use short-lived HMAC-signed backend URLs bound to project, work item, attachment, and expiry, then re-check visibility and content policy before streaming bytes.
...
- Program reads require workspace `workspace.read`. Program create/update/archive and program-project assignment/removal require workspace `workspace.admin`; assigned projects must be active and belong to the same workspace.
...
- Current-user notification list/read and notification preference APIs require workspace `workspace.read` and only expose or mutate rows owned by the current user. Direct notification creation requires workspace `workspace.admin` and can target only active members of that workspace.
- Automation rule, automation child configuration, manual execution, queued worker execution, scheduled worker settings, worker run/health reads, worker retention export/prune, automatic worker-run pruning, workspace email provider settings, webhook configuration, webhook delivery reads/admin, email delivery reads/admin, and delivery processing APIs require workspace `automation.admin`. Rule execution queues jobs first. Worker execution creates in-app notifications synchronously, creates workspace-provider-backed email delivery rows, writes queued webhook delivery rows, records run-history/health rows for automation/webhook/email workers, and import conflict-resolution worker execution records the same run/health rows when scheduled or run manually. Worker retention export/prune can be narrowed by `workerType`, `triggerType`, `status`, and `startedFrom`/`startedTo` for operations-safe cleanup. Webhook and email delivery processing support dry-run or real signed delivery, bounded retries, cancel/retry administration, and `dead_letter`.
- Import job list/detail/lifecycle/parse/sample/record/template/preset/version/materialization/conflict/rerun/diff/import-review-export/conflict-resolution-job APIs require workspace `workspace.admin`. CSV, Jira JSON, and Rally JSON parsers create reviewable import records; admins can filter records by status/conflict/source type, edit imported records before materialization, inspect append-only record version history plus backend-normalized record and job-level version diff rows, create backend-generated sample import jobs when local/dev or explicitly deployment-enabled, and create stored import diff and review-table CSV export artifacts through the generic export-job system. Mapping templates can create or update work items through the existing work item service after applying transform presets, template-local transformation config, value lookups, type translations, and status translations, preserving project permissions, sequence allocation, workflow defaults, screen/custom-field validation, history writes, and domain events. Conflict resolution can mark a record skipped, stage it for update-existing processing, or clear the target so a rerun can create a new work item; single-record, selected bulk, and filtered bulk resolution use the same validation, with filtered bulk guarded by paginated preview, `expectedCount`, typed confirmation, and a server-side synchronous apply cap. Larger filtered sets can be stored as queued conflict-resolution jobs, then run, canceled, retried, processed by a bounded worker, or picked up by workspace worker settings through the same per-record conflict-resolution lifecycle while retaining matched/resolved/failed counters. Completing a job with open conflicts requires `acceptOpenConflicts=true`, the `COMPLETE WITH OPEN CONFLICTS` phrase, and a non-empty audit reason, which is stored on the import job for later reporting/dashboard summaries and focused import completion report endpoints.
- Attachment metadata/upload/download, generated export artifacts/downloads, and import parser payloads enforce effective security policy byte limits and allowlisted content types. Effective limits merge deployment defaults, workspace overrides, and project overrides where the operation has project context, such as work item attachments and project-targeted import jobs/exports. Workspace admins can override deployment defaults per workspace, project admins can override inherited limits per project, and multipart request size limits remain environment-configurable.
- Activity streams use service-layer permission checks for the requested workspace, project, or work item.
- System-admin list/grant/revoke APIs require active dedicated system-admin access and refuse to remove the last active system admin; production-like grant/revoke additionally requires recent authentication inside the configured step-up window. Workspace security policy APIs require workspace `workspace.admin`, project security policy APIs require project `project.admin`, workspace role management requires `user.manage`, and project role management requires project `project.admin`. Role permission saves require impacted-member/invitation preview confirmation, role versions are immutable rollback snapshots, archived roles are hidden from normal assignment flows, and last-admin checks prevent removing the only route to workspace/project administration. Audit log, audit retention, and domain event replay APIs are workspace-admin-only through method-level permission checks.
...
- Team and planning APIs expose the core configuration that reporting and work item assignment depend on. Workspace admins can create/update/archive teams and manage immediate membership joins/leaves. Project admins can assign active teams to projects and update the current project-team role. Board admins can manage boards, columns, workflow-status-backed column mappings, saved-filter/query-backed swimlanes, releases, release scope, project-scoped roadmaps, roadmap items, project-scoped iterations, iteration scope, commitments, closes, and incomplete-work carryover. Workspace admins can manage workspace-scoped roadmaps. Board work item queries return real cards grouped by configured column status mappings and configured swimlanes, and now also accept optional iteration/team/view-mode filters so the frontend can render active-sprint or backlog scopes from the same board definition; board-scoped rank/transition/move endpoints validate board membership before applying work item commands. Board transition and move requests can derive the workflow transition from target board column/status mappings, and move requests can update rank in the same request using card-level relative positions.
...
- The frontend Work route exposes the collaboration APIs through inline work item detail controls: comment create/edit/delete, fixed and custom link-type creation, self-service watching, work-log create/edit/delete, workspace label creation, work-item label assignment/removal, drag/drop and file-picker attachment upload, attachment download/delete, and activity review.
...
- Reporting read APIs expose status history, assignment history, team history, estimate history, non-deleted work-log totals, on-demand project dashboard summaries, workspace/program cross-project dashboard summaries, focused project/workspace import completion metrics, raw persisted snapshot records, report-shaped raw and rollup snapshot series, and live/snapshot iteration reports. Project summaries return work counts, throughput, estimate/time totals, aging WIP, cycle-time inputs, import completion/open-conflict acceptance metrics, status/type/priority groupings, and widget-shaped payloads for project, team, and iteration scopes. Workspace/program summaries expose project, team, work item type, and import completion dimensions and order project comparisons by earliest `workspace_sequence_number`. Dashboard rendering supports project and portfolio import-completion widget types. Snapshot jobs populate `cycle_time_records`, `iteration_snapshots`, `velocity_snapshots`, and `cumulative_flow_snapshots`; manual run, backfill, and reconcile controls are implemented. Snapshot retention policy APIs, dedicated daily/weekly/monthly rollup tables, manual rollup/backfill controls, and archive-run metadata are implemented; destructive raw snapshot pruning remains future work. Direct project/team list endpoints exist for dashboards, saved filters, and governed report query catalog entries. Governed report query catalog entries validate widget runtime parameters against `parameters_schema` before reporting services execute.

### Import And Worker Operations

- Import transform presets are reusable workspace records that store named `transformation_config` pipelines and increment a `version` on update or disable. Each create/update/disable writes an immutable `import_transform_preset_versions` row that is exposed through REST and the frontend as first-class version history. Historical versions can be cloned into standalone new presets so rollback-like recovery does not silently change templates that still point at the current preset. Historical versions can also be cloned and retargeted to selected mapping templates through separate preview/apply APIs. Import mapping templates may reference one preset through `transform_preset_id` and may still define template-local transformation config. During materialization, preset transforms are merged first and template-local transforms override matching target fields. Each materialization writes `import_materialization_runs` with mapping-template, mapping-rule, transform-preset, and merged-transform snapshots so completed imports remain reproducible after a preset or mapping rule changes. Snapshot reruns default to the source run's `updateExisting` value for reproducibility, but admins can explicitly override that flag for a modified rerun while keeping the stored template/rule/transform inputs.
- Import mapping templates store `field_mapping`, `defaults`, optional `transform_preset_id`, and optional local `transformation_config`. Transformation config is a target-field keyed JSON object whose values can be a legacy function name, an ordered array of function names, a single transform step object, or an object with a `pipeline` array. Supported steps include `trim`, `lower`/`lowercase`, `upper`/`uppercase`, `collapse_whitespace`, `replace`, `prefix`/`prepend`, `suffix`/`append`, `truncate`, and `substring`; step arguments can be placed at the top level or under `args`. Preset/template save paths validate unsupported transform functions up front so invalid mappings do not silently no-op.
...
- Mapping materialization still uses `WorkItemService`, so imports preserve normal validation, workflow defaults, screen/custom-field checks, history writes, domain events, and permissions instead of bypassing core work item behavior. Re-materializing imported records with `updateExisting=false` is idempotent: existing targets are skipped, counted as skipped/conflicted, and surfaced on the materialization response/run rather than creating duplicates. Open conflicts are queryable through dedicated APIs and can be resolved individually, in selected bulk batches, by current filters as `skip`, `update_existing`, or `create_new`, or through persisted filtered conflict-resolution jobs for sets larger than the synchronous cap; `update_existing` keeps the target attached but stages the record as pending, and unresolved `conflict` records are not materialized until an admin resolves or edits them. Filtered bulk resolution always targets open conflicts, requires preview through `/conflicts/resolve-preview`, returns paginated preview rows with total matched counts, and synchronous apply only succeeds with matching `expectedCount` plus exact `RESOLVE FILTERED CONFLICTS` confirmation while staying under the server-side batch cap. Async conflict-resolution job creation uses the same filters, expected count, and typed confirmation, then stores queued/running/completed/failed/cancelled counters for large review sets. Jobs can be run individually, canceled while queued/failed, retried after failure/cancellation, listed at workspace scope by status, processed through a bounded worker endpoint, or picked up by the shared scheduled worker when `automation_worker_settings.import_conflict_resolution_enabled` is true; the scheduled worker uses `import_conflict_resolution_limit` and defaults off. Record list APIs support `status`, `conflictStatus`, and `sourceType` filters. Reparsing or editing a source record clears stale conflict metadata and returns the record to review. Completing an import job with open conflicts is blocked unless the request includes `acceptOpenConflicts=true`, exact `COMPLETE WITH OPEN CONFLICTS` confirmation, and a non-empty audit reason that is written both to the completion event payload and first-class import job reporting fields.
- `import_job_record_versions` stores append-only snapshots for record lifecycle changes including `created`, `updated`, `parsed`, `reparsed`, `materialized_created`, `materialized_updated`, `materialization_failed`, `conflict_opened`, and `conflict_resolved`. The backend exposes normalized adjacent field-level diff rows through `GET /api/v1/import-job-records/{recordId}/version-diffs`, job-level grouped diff/export rows through `GET /api/v1/import-jobs/{importJobId}/version-diffs` and `/version-diffs/export`, and durable stored JSON or filtered CSV artifacts through `POST /api/v1/import-jobs/{importJobId}/version-diffs/export-jobs` plus the generic export-job list/download APIs; the frontend displays those backend-owned rows beside raw snapshots so admins can inspect what changed during parsing, review, materialization, and conflict resolution.
- `POST /api/v1/workspaces/{workspaceId}/import-review/export-jobs` creates queued durable CSV export-job artifacts for import review tables that need audit/download history: conflict-resolution jobs, import export-artifact history, project import-completion metrics, and workspace import-completion metrics. The request carries `tableType`, optional `importJobId`, optional `projectId`, optional status/export-type narrowing, and the frontend's current column filter state. `POST /api/v1/workspaces/{workspaceId}/import-review/export-jobs/process` lets admins process a bounded batch of queued review CSV exports and attach completed artifacts through configured attachment/object storage. Queued import review export jobs can also be picked up by the shared scheduled worker when `automation_worker_settings.import_review_exports_enabled` is true; scheduled/manual runs write `import_review_export` worker run and health rows.
- `GET /api/v1/workspaces/{workspaceId}/import-settings` and `PATCH /api/v1/workspaces/{workspaceId}/import-settings` expose workspace-admin import feature toggles. `GET /api/v1/workspaces/{workspaceId}/import-samples` exposes the backend sample catalog, and `POST /api/v1/workspaces/{workspaceId}/import-samples/{sampleKey}/jobs` creates a demo import job, parses the sample payload, and can optionally create a reusable transform preset plus mapping template/type/status/value-lookup rules for a selected project. Sample endpoints remain workspace-admin-only, require the workspace `sampleJobsEnabled` setting, and production-like `prod`, `production`, `staging`, and `hosted` profiles also require `TRASCK_IMPORT_SAMPLE_JOBS_ENABLED=true`.
- Import lifecycle, transform preset, materialization, conflict-resolution job, and diff export artifact events are projected into activity/audit streams through the shared import event prefix.
- Automation, webhook, email, and import conflict-resolution worker endpoints write `automation_worker_runs` rows for both manual and scheduled executions. Runs record trigger type, dry-run flag when applicable, requested limits, max attempts when applicable, processed/success/failure/dead-letter counts, actor metadata for manual runs, and start/finish times. Operations APIs can filter run history and health by worker type.
- Worker run retention settings live in `automation_worker_settings`. Admins can export retention candidates to configured attachment/object storage, create `export_jobs`, and manually prune eligible worker run rows after optional export. Manual export/prune accepts optional `workerType`, `triggerType`, `status`, and `startedFrom`/`startedTo` filters so one worker family or operational slice can be cleaned up without deleting unrelated worker history. Workspaces may also opt into automatic scheduled pruning through `worker_run_pruning_automatic_enabled`; automatic pruning uses the same export-before-prune safeguards, records system-authored prune events, runs on a dedicated scheduler, and honors per-workspace `worker_run_pruning_interval_minutes`, optional `worker_run_pruning_window_start`/`worker_run_pruning_window_end`, and last-start/last-finish timestamps. Agent dispatch-attempt retention also has workspace-level defaults and optional scheduled pruning through `agent_dispatch_attempt_*` worker settings, reusing the existing export-before-prune dispatch-attempt retention flow.
...
- `automation_worker_health` stores the latest run status by workspace and worker type plus consecutive failure counts, including `import_review_export`, giving the frontend an operations summary without scanning every delivery/job row.
...
- `AgentProviderAdapter` is the adapter contract used by provider-specific implementations. Current implementations are `simulated`, `codex`, `claude_code`, and `generic_worker`. Simulated and generic worker remain non-external. Codex and Claude Code default to non-external `stub` mode, but can be intentionally configured for `runtime.mode=hosted_api` or `runtime.mode=cli_worker` when `externalExecutionEnabled=true`; these modes emit provider-neutral dispatch payloads. Hosted API remains a boundary/preview mode. Backend-managed CLI execution is implemented for Codex and Claude Code, gated by `TRASCK_AGENTS_CLI_WORKER_ENABLED=false` by default, and normal automated tests exercise it with a local shell stand-in instead of paid/vendor CLIs. Contract tests cover dispatch, retry, cancel, idempotency metadata, hosted API boundaries, CLI-worker allowlist enforcement, and generic-worker payload guarantees.
...
- Agent providers currently accept `simulated`, `codex`, `claude_code`, and `generic_worker` provider types. The Codex and Claude Code adapters expose the provider-neutral runtime payload for internal stub, provider-hosted API, and hardened CLI-worker transport boundaries. CLI-worker configuration accepts only named command profiles, not raw commands. The backend-managed CLI dispatcher runs only allowlisted `codex-local` and `claude-code-local` command profiles, writes `prompt.md` and `task.json` into a per-task run directory, injects the selected encrypted provider credential through a configured environment variable, captures bounded redacted stdout/stderr, records `cli_worker_started`/`cli_worker_completed`/`cli_worker_failed` task events, and uses the normal signed callback path to move tasks to review or failed status. Local manual-test defaults are edit-capable for the current human test round: Codex uses `codex exec --full-auto -`, and Claude Code uses `claude -p {prompt} --permission-mode bypassPermissions`; the worker remains disabled by environment unless explicitly enabled on a trusted host. Workspace admins can list CLI run metadata, download completed/review-requested run ZIPs, delete a single run directory, and prune old non-active run directories through admin-only APIs and the Agent page. The generic worker adapter establishes a provider-neutral protocol payload (`trasck.worker.v1`) and supports both pull polling and durable outbound webhook push through database-configured domain-event consumers without coupling work item code to a vendor.
- `POST /api/v1/agent-providers/{providerId}/runtime-preview` validates and previews a provider runtime payload for `stub`, `hosted_api`, or `cli_worker` without dispatching work. The response is redacted and uses plain JSON values so provider private keys and encrypted secrets never leak through runtime preview.
- Dispatch, retry, and cancel calls across agent adapters are recorded in `agent_dispatch_attempts`, including provider/profile/task/work item references, attempt type, dispatch mode, provider type, transport, success/failure status, external task ID, idempotency key, external-dispatch flag, request/response payloads, error message, and timing. Agent task API responses include these attempts in stable chronological order so admins can inspect the adapter boundary before enabling real external execution. Workspace admin endpoints `GET /api/v1/workspaces/{workspaceId}/agent-dispatch-attempts`, `POST /api/v1/workspaces/{workspaceId}/agent-dispatch-attempts/export`, and `POST /api/v1/workspaces/{workspaceId}/agent-dispatch-attempts/prune` provide filtered list, storage-backed JSON export, and export-before-prune retention flows for dispatch-attempt history. CLI worker artifact endpoints `GET /api/v1/workspaces/{workspaceId}/agent-cli-runs`, `GET /api/v1/workspaces/{workspaceId}/agent-cli-runs/{agentTaskId}/download`, `DELETE /api/v1/workspaces/{workspaceId}/agent-cli-runs/{agentTaskId}`, and `POST /api/v1/workspaces/{workspaceId}/agent-cli-runs/prune` let workspace admins inspect, export, and clean up local run directories without direct filesystem access.
- Provider credentials and callback private keys are stored as AES-GCM encrypted database secrets through `SecretCipherService`. Admin APIs can list redacted credentials, deactivate non-callback credentials, re-encrypt provider credentials with the current cipher, and rotate per-provider callback signing keys. Agent provider credentials support optional `expires_at`, and expired worker tokens are rejected. Codex credentials are limited to `codex_api_key` or `codex_cli_token` for external provider credentials, and Claude Code credentials are limited to `anthropic_api_key` or `claude_cli_token`; shared `callback_signing` and `worker_token` credentials remain provider-neutral. External provider credential secrets must meet basic length/whitespace checks and optional `metadata.authScheme` is constrained to supported values. The first implementation is database-backed and intentionally abstracted so Vault/KMS/Secrets Manager can replace the cipher/storage path later without changing agent callback logic.
...
erDiagram
    ...
    WORKSPACES ||--o{ IMPORT_CONFLICT_RESOLUTION_JOBS : queues
    IMPORT_JOBS ||--o{ IMPORT_CONFLICT_RESOLUTION_JOBS : resolves
    WORKSPACES ||--o{ IMPORT_TRANSFORM_PRESETS : defines
    IMPORT_TRANSFORM_PRESETS ||--o{ IMPORT_TRANSFORM_PRESET_VERSIONS : versions
    ...
    IMPORT_JOB_RECORDS ||--o{ IMPORT_JOB_RECORD_VERSIONS : versions
    IMPORT_JOBS ||--o{ IMPORT_MATERIALIZATION_RUNS : snapshots
    ...
    WEBHOOKS {
        ...
        string previous_secret_hash
        string previous_secret_encrypted
        string previous_secret_key_id
    ...
    IMPORT_JOBS {
        ...
        boolean open_conflict_completion_accepted
        integer open_conflict_completion_count
        uuid open_conflict_completed_by_id FK
        timestamptz open_conflict_completed_at
        text open_conflict_completion_reason
    }
    ...
    IMPORT_CONFLICT_RESOLUTION_JOBS {
        ...
        string conflict_status_filter
        string source_type_filter
    ...
    IMPORT_TRANSFORM_PRESET_VERSIONS {
        uuid id PK
        uuid preset_id FK
        uuid workspace_id FK
        integer version
    ...
    IMPORT_MATERIALIZATION_RUNS {
        uuid id PK
        uuid workspace_id FK
        uuid import_job_id FK
        uuid mapping_template_id FK
        uuid transform_preset_id FK
        integer transform_preset_version
        uuid project_id FK
        uuid requested_by_id FK
        boolean update_existing
        jsonb mapping_template_snapshot
        jsonb transform_preset_snapshot
        jsonb transformation_config_snapshot
        jsonb mapping_rules_snapshot
        string status
    ...
    IMPORT_JOB_RECORDS {
        ...
        timestamptz conflict_resolved_at
        string conflict_resolution
        uuid conflict_resolved_by_id FK
        uuid conflict_materialization_run_id FK
    }

    IMPORT_JOB_RECORD_VERSIONS {
        uuid id PK
        uuid import_job_record_id FK
        uuid import_job_id FK
        integer version
        string change_type
...
erDiagram
    ...
    AGENT_TASKS ||--o{ AGENT_DISPATCH_ATTEMPTS : attempts
...
| `roles` | Named permission bundle scoped to workspace or project, with active/archived lifecycle state. |
| `permissions` | Individual capability keys such as `work_item.create`. |
| `role_permissions` | Join table for roles and permissions. |
| `role_versions` | Immutable role snapshots used for permission/metadata history and rollback. |
...
| `automation_worker_settings` | Per-workspace switches and limits for scheduled automation, webhook, email, import conflict-resolution, and retention workers. |
| `automation_worker_runs` | Per-run history for manual and scheduled automation, webhook, email, and import conflict-resolution worker processing. |
...
| `webhooks` | Outbound webhook definitions with redacted secret hash, encrypted secret material, and non-secret signing key ID for delivery signing and rotation. |
...
| `agent_dispatch_attempts` | Durable dispatch/retry/cancel adapter-boundary attempts for agent tasks. |
...
| `import_workspace_settings` | Per-workspace import feature toggles such as guarded sample job availability. |
| `import_jobs` | Jira/Rally/CSV import jobs. |
| `import_conflict_resolution_jobs` | Queued filtered conflict-resolution jobs for large import review sets. |
| `import_transform_presets` | Named reusable versioned workspace transformation pipelines for import mappings. |
| `import_transform_preset_versions` | Immutable per-version snapshots for reusable import transform presets. |
| `import_materialization_runs` | Snapshot records for each import materialization attempt, including exact rerun inputs. |
...
| `import_job_records` | Per-record import results and conflict review state. |
| `import_job_record_versions` | Append-only per-record import lifecycle and edit snapshots. |
...
| `roles` | `id`, `workspace_id`, `project_id`, `name`, `key`, `scope`, `description`, `system_role`, `status`, `archived_at`, `created_at`, `updated_at` |
| `permissions` | `id`, `key`, `name`, `description`, `category` |
| `role_permissions` | `role_id`, `permission_id`, `created_at` |
| `role_versions` | `id`, `role_id`, `workspace_id`, `project_id`, `version_number`, `name`, `key`, `scope`, `description`, `system_role`, `status`, `permission_keys`, `change_type`, `change_note`, `created_by_id`, `created_at` |
...
| `programs` | `id`, `workspace_id`, `name`, `description`, `status`, `roadmap_config`, `report_config`, `created_at`, `updated_at` |
...
| Table | Important Columns |
|---|---|
| `board_swimlanes` | `id`, `board_id`, `name`, `swimlane_type`, `saved_filter_id`, `query`, `position`, `enabled` |
...
| `automation_worker_settings` | `workspace_id`, `automation_jobs_enabled`, `webhook_deliveries_enabled`, `email_deliveries_enabled`, `import_conflict_resolution_enabled`, worker limits including `import_conflict_resolution_limit`, max attempts, dry-run switches, worker run retention settings, `worker_run_pruning_automatic_enabled`, `worker_run_pruning_interval_minutes`, pruning window and last-run timestamps, `updated_at` |
| `automation_worker_runs` | `id`, `workspace_id`, `worker_type` (`automation`, `webhook`, `email`, `import_conflict_resolution`), `trigger_type`, `status`, `dry_run`, `requested_limit`, `max_attempts`, processed/success/failure/dead-letter counts, `error_message`, `metadata`, `started_at`, `finished_at` |
...
| `webhooks` | `id`, `workspace_id`, `name`, `url`, `secret_hash`, `secret_encrypted`, `secret_key_id`, previous-secret hash/encrypted/key ID overlap fields, `secret_rotated_at`, `previous_secret_expires_at`, `previous_secret_overlap_seconds`, `event_types`, `enabled` |
| `webhook_deliveries` | `id`, `webhook_id`, `event_type`, `signature_key_id`, `payload`, `status`, `response_code`, `response_body`, `attempt_count`, `next_retry_at`, `created_at` |
...
| `agent_dispatch_attempts` | `id`, `workspace_id`, `agent_task_id`, `provider_id`, `agent_profile_id`, `work_item_id`, `requested_by_id`, `attempt_type`, `dispatch_mode`, `provider_type`, `transport`, `status`, `external_task_id`, `idempotency_key`, `external_dispatch`, `request_payload`, `response_payload`, `error_message`, `started_at`, `finished_at` |
...
| `import_workspace_settings` | `workspace_id`, `sample_jobs_enabled`, `created_at`, `updated_at` |
| `import_jobs` | `id`, `workspace_id`, `requested_by_id`, `provider`, `status`, `config`, `started_at`, `finished_at`, `open_conflict_completion_accepted`, `open_conflict_completion_count`, `open_conflict_completed_by_id`, `open_conflict_completed_at`, `open_conflict_completion_reason` |
| `import_conflict_resolution_jobs` | `id`, `workspace_id`, `import_job_id`, `requested_by_id`, `resolution`, `scope`, `status`, filter fields, `expected_count`, `matched_count`, `resolved_count`, `failed_count`, `error_message`, `confirmation`, `requested_at`, `started_at`, `finished_at` |
| `import_transform_presets` | `id`, `workspace_id`, `name`, `description`, `transformation_config`, `enabled`, `version`, `created_at`, `updated_at` |
| `import_transform_preset_versions` | `id`, `preset_id`, `workspace_id`, `version`, `name`, `description`, `transformation_config`, `enabled`, `change_type`, `created_by_id`, `created_at` |
...
| `import_materialization_runs` | `id`, `workspace_id`, `import_job_id`, `mapping_template_id`, `transform_preset_id`, `transform_preset_version`, `project_id`, `requested_by_id`, `update_existing`, `mapping_template_snapshot`, `mapping_rules_snapshot`, `transform_preset_snapshot`, `transformation_config_snapshot`, status/counts including skipped/conflicted, `created_at`, `finished_at` |
...
| `import_job_records` | `id`, `import_job_id`, `source_type`, `source_id`, `target_type`, `target_id`, `status`, `error_message`, `raw_payload`, `conflict_status`, `conflict_reason`, `conflict_detected_at`, `conflict_resolved_at`, `conflict_resolution`, `conflict_resolved_by_id`, `conflict_materialization_run_id` |
| `import_job_record_versions` | `id`, `import_job_record_id`, `import_job_id`, `version`, `change_type`, `changed_by_id`, `snapshot`, `created_at` |
...
agent_dispatch_attempts(workspace_id, started_at)
agent_dispatch_attempts(agent_task_id, started_at, id)
agent_dispatch_attempts(provider_id, status, started_at)
...
POST   /api/v1/agent-providers/{providerId}/deactivate
...
POST   /api/v1/agents/{profileId}/deactivate
...
- `import_jobs`, `import_conflict_resolution_jobs`, `import_transform_presets`, `import_mapping_templates`, import mapping rule tables, `import_materialization_runs`, `import_job_records`, and `import_job_record_versions` provide observable import progress, guarded backend sample job creation, reusable source mappings, reusable transformation pipelines, transformation/translation rules, materialization output, exact rerun snapshots, conflict review, queued large filtered conflict resolution, completion-audit/reporting fields, lifecycle history, normalized record/job diff rows, durable import review CSV export artifacts, and error reporting.
...
- The first provider adapters are `simulated`, `codex`, `claude_code`, and `generic_worker`. Simulated and generic worker prove the provider-neutral lifecycle without vendor-specific execution. Codex and Claude Code now support env-gated backend-managed CLI-worker execution for trusted manual testing while keeping stubs as the default, and workspace admins can list/download/delete/prune generated CLI run artifacts. The generic worker protocol supports polling, manual push-payload generation, and durable webhook push through configured domain-event consumers; hosted provider APIs and future agents should plug into the same contract later.
...
- `agent_dispatch_attempts` record each dispatch/retry/cancel adapter call with request/response payloads, idempotency keys, external task IDs, status, and failure diagnostics.
...
7. Implement comments, links, watchers, labels, basic attachment metadata, work logs, attachment byte storage, activity events, and audit logs. The comment/link/watcher/work-log/label/attachment storage slice, including workspace label delete cleanup, activity projections, audit projections, retention policy metadata, and replay wiring are implemented.
...
11. Implement notifications, automation, webhooks, and integrations. Current-user notification/preference APIs, workspace default notification policy APIs, automation rule/condition/action/job APIs, queued automation worker endpoints, worker run history/health/export/prune APIs with worker type, trigger type, status, and started-at range filters, optional automatic worker-run pruning with dedicated interval/window throttling, webhook configuration plus queued delivery records, webhook delivery processing with retry/dead-letter, workspace Maildev/SMTP email provider settings, workspace-provider-backed email delivery rows, import job lifecycle/record/parser/edit APIs for CSV/Jira/Rally, import workspace settings, import record lifecycle version snapshots and normalized backend diff rows, job-level import diff/export APIs plus stored JSON/CSV export-job artifacts, queued backend-generated CSV export-job artifacts for conflict-resolution job/export-artifact/completion review tables with bounded admin or scheduled worker processing, reusable versioned import transform presets with immutable version records, clone-from-version recovery, clone-and-retarget preview/apply, import materialization with preset/template transformation pipelines, mapping-rule snapshots, exact and modified rerun from materialization snapshots, skipped/conflicted idempotency counters, filtered record lists, dedicated single/selected/filtered conflict review/resolve APIs with paginated preview and batch caps, queued filtered conflict-resolution jobs for larger sets with cancel/retry/manual run/bounded worker pickup, workspace-settings-controlled scheduled conflict-resolution and import review export pickup with dedicated worker run/health rows, focused import completion report endpoints/widgets, guarded completion with typed open-conflict confirmation and first-class audit/report/dashboard fields, executable full-flow sample import fixtures, deployment-plus-workspace-gated backend-generated sample import jobs/mappings, sample import payloads, and value/type/status mapping rules are implemented.
12. Implement provider-neutral AI agent foundations: provider adapters, agent profiles, repository connections, agent tasks, dispatch attempts, callbacks, messages, artifacts, permissions, and audit events. The simulated/Codex/Claude Code/generic-worker service/API slice is implemented with project-scoped profiles, explicit provider/profile deactivate APIs, visible deactivated provider/profile admin records for auditability, repository connection create/list/deactivate APIs, encrypted provider secrets, provider-specific Codex/Claude credential validation, encrypted per-provider callback signing keys, per-worker token polling/webhook protocol endpoints, durable outbound worker webhook delivery through configured domain-event consumers, provider-level max-attempt/dead-letter settings, persisted dispatch/retry/cancel attempt rows, workspace dispatch-attempt list/export/prune endpoints, workspace CLI run artifact list/download/delete/prune endpoints, workspace-level dispatch-attempt retention defaults with optional automatic scheduled pruning, optional worker-token expiry, request-changes/human-message flows, credential re-encryption, credential deactivation, immediate callback key invalidation on rotation, hosted API boundaries, backend-managed edit-capable Codex/Claude CLI-worker execution behind explicit external-execution and environment opt-in, runtime preview, and adapter contract tests. Real vendor CLI/API execution remains disabled from normal automated tests; test coverage uses local command-profile stand-ins.
...
14. Implement the Vite React frontend. The first route-based console flows are implemented with local API-base configuration, HTTP-only-cookie browser auth plus CSRF using `X-XSRF-TOKEN`, no access-token local storage, per-unsafe-request CSRF refresh so login/setup/logout session changes do not leave stale browser headers behind, global toast/error handling, feature-specific services around the generated client, setup availability checks, setup/login/current-user actions, post-setup homepage redirect, permission-aware tabs/routes/page loads derived from `/api/v1/auth/context` workspace/project `permissionKeys` plus `systemAdmin`, admin/API token creation/listing, system-admin grant/revoke/list controls, Workspace Settings security-policy/member/invitation management with full workspace role management, Project Settings security-policy/public-read effective state controls with full project role management and a public preview link, a Programs route for workspace program CRUD/project assignment/dashboard summary review, a dedicated anonymous public project/work-item/comment/attachment preview route with signed public attachment downloads, project work item cursor list/detail/create plus inline collaboration controls for comments, links, watchers, work logs, labels, attachment upload/download/delete, and activity review, dedicated planning Overview/Backlog/Active Board/Admin routes with iteration assignment/removal, sprint-start actions, iteration/team-scoped board rendering, board detail routes, workflow-status-backed board column configuration, and separate team/board/release/roadmap administration, configuration screens and detail routes, notification/automation/webhook worker screens and detail routes, worker run/health previews with worker-type filters, worker retention export/prune/automatic schedule controls including worker type, trigger type, status, and date-range filtered export/prune calls, import conflict scheduled-worker settings, import job/template/detail routes with transform preset CRUD, transform preset version-history display, clone-from-version and clone-retarget controls, mapping preset selection, materialization run display, exact/modified rerun controls, open conflict detail review/resolve controls, selected and filtered bulk conflict resolution, queued filtered conflict-resolution job cancel/retry/manual run/worker controls, workspace import ops audit tables, workspace import sample setting controls, sample import walkthrough creation, record filtering that waits for parsed/reviewable import jobs, backend-provided field-specific import record/job version-diff tables with column filtering, backend-generated queued filtered job-diff/conflict-job/export-artifact/completion CSV export jobs, stored import review export artifact creation/process/history/download controls, import completion report tables with column filtering and CSV export, import record edit/version-history forms, field-specific import/planning/agent/collaboration controls for the common flows that previously used raw JSON/ID inputs, custom-field transform pipeline controls with JSON escape hatches, and mapping rule creation, saved filter/view builder and saved-filter execution, dashboard/widget builder and render, agent provider/profile/repository/task flows with Codex/Claude presets, provider credential list/create/deactivate/re-encrypt controls, callback-key rotation, runtime preview, visible active/deactivated provider/profile status markers, dispatch-attempt admin list/export/prune controls, and CLI run artifact list/download/delete/prune controls, selector-fed common IDs, board card rendering and card-level drag/drop moves, manual board move controls, checked-in generated OpenAPI/client artifacts refreshed from the live backend contract, and Vitest/Testing Library route smoke/component coverage.
15. Build the external Java Playwright automation suite in `../trasck-test`. The initial harness is implemented with Maven, Java Playwright, environment-backed backend/frontend base URLs, optional login credentials plus workspace/project IDs for authenticated coverage, optional first-run disposable setup bootstrap through `/api/v1/setup`, separate opt-in sample-data fixture support plus direct per-test seed creation for real-backend browser workflows, opt-in managed production-like backend stack coverage using Testcontainers PostgreSQL/Redis plus a child backend process from the sibling `trasck` repo, Docker support, reusable API/browser helpers, a local HTTP receiver for webhook and worker delivery assertions, deterministic local-stack guidance, committed OpenAPI route coverage baseline enforcement with per-covered-route `coverageOwner`, generated OpenAPI route inventory output, redacted API snippets, screenshots, traces, backend API smoke tests for health, CSRF metadata, setup endpoint reachability and first-run-only conflict behavior, unauthenticated current-user denial, optional login/current-user cookie and Bearer sessions, local OpenAPI availability, managed production startup rejection coverage for unsafe defaults and unreachable Redis-backed rate limiting, managed production OpenAPI/Swagger protection, secure auth-cookie, system-admin list/grant/revoke, last-active-admin, recent-login step-up, and non-admin denial coverage, authenticated API coverage for workspace/project security policy and public project/work-item/comment/attachment read boundaries with signed public attachment downloads, personal and service token scope behavior, system-admin/audit/export reads and mutation coverage, domain-event replay coverage, configuration/custom-field/screen/report-query/reporting-retention CRUD, work item collaboration/activity including label cleanup and attachment byte upload/download, reporting history/snapshot reads and snapshot execution controls, program CRUD/project assignment/dashboard summary coverage, team-scoped dashboard/filter/view/report-query lists, repository connections, invitation list/cancellation, workspace-admin human user list/creation/removal, full workspace/project role management, teams/planning/boards/releases/roadmaps, notification preferences/defaults plus direct and automation-created notification read coverage, automation async rule/job/webhook/email/worker API coverage with signed real local webhook receiver assertions including dual-secret rotation overlap and durable delivery key IDs, high-risk import lifecycle/review-export/report API coverage, auth/logout/register/OAuth unsafe-input coverage, provider-neutral agent provider/profile/credential/task/callback/worker-token/dispatch-attempt/CLI-run-artifact API coverage, dashboards, saved filters, saved views, favorites, and recent items, cleanup through public APIs where cleanup APIs exist, frontend browser smoke tests for the shell, Workspace Settings member/invitation flow, program/portfolio management, automation async worker controls, auth/token/system-admin controls, import review/export worker controls, agent provider/profile/repository/task/dispatch-attempt/CLI-run controls, public-read settings, and public project/work-item/comment/attachment preview route, real-backend seeded browser workflows that sign in through the UI, verify no access token is stored in browser local storage, load/create work items, execute saved filters, render dashboards, create a team, create a custom field, create planning/board/release/roadmap records through the frontend, load board detail through the frontend, create import jobs/records/materialization through the frontend without known import-record console ignores, exercise agent provider/profile/repository/task/dispatch-attempt flows with explicit provider/profile deactivation, create work items and collaboration records through the Work route UI, verify public preview visibility for public-safe comments and attachments, and split remaining frontend coverage into feature-specific import, automation/worker, notification, configuration/search, dashboard, program/admin/token, public-preview, and work-item collaboration workflows. The suite also has a disposable first-run route rehearsal and longer planning/import/automation/agent creation scenario that create their own setup context on an empty stack. The checked-in GitHub Actions workflow compiles the Java suite and runs the Docker-backed `managed-stack` production security group on pull requests and `main` pushes when the required `TRASCK_BACKEND_REPOSITORY` repository variable is set. This suite should become the full-stack regression home for implemented backend APIs, frontend workflows, security audit checks, worker/import/export/agent flows, and cross-system scenarios. The committed route baseline currently tracks 403 covered routes, 0 ordinary planned routes, and 0 planned high-risk routes.

## Testing Requirements

Current backend coverage includes PostgreSQL/Testcontainers verification for Flyway/JPA persistence, richer core JPA associations, the first setup flow, deterministic seed data, anonymous public project/work-item/comment/attachment visibility including signed public attachment downloads, first-run-only setup enforcement, username/password login, cookie and Bearer-token auth, cookie CSRF enforcement, generated OpenAPI availability, production-like Redis rate-limit startup validation, system-admin docs gating/list/grant/revoke/recent-login safeguards, workspace and project security-policy enforcement, database and Redis auth-throttle paths, redaction utility coverage, personal API token creation/authentication/listing/revocation/expiry, service token creation/authentication/listing/expiry, token scope denial, workspace and project-specific invitation registration, workspace-admin user creation, full OAuth redirect startup, signed OAuth identity linking, provider-specific verified-email resolution rules, static and configured durable event delivery publication with retry/dead-letter/disabled/unknown-replay/multi-consumer coverage, permission denial, watcher self-service, dedicated-permission self-service work logs, team CRUD/membership/project-team APIs, program CRUD/project assignment APIs, board/column/swimlane APIs, saved-filter/query-backed swimlane rendering, saved-filter-ID swimlane rendering, per-column and swimlane-grouped board work item queries, board-scoped rank/transition/move APIs, drag/drop transition derivation from target board column/status mappings, card-level board insertion validation, board move relative-card edge cases, release/scope APIs, roadmap/item APIs, iteration CRUD/scope/commit/close/carryover APIs, cursor-page project work item lists, typed searchable custom-field filtering, saved-filter multi-predicate work item execution with custom/system predicates, due-date and priority sort coverage, invalid operator coverage, project-scoped permission coverage, and cursor pagination, screen-driven required field enforcement on work item create/update, targeted required-field enforcement on team and assignee command clears, cursor-page activity/audit/export-job lists, work-item reporting/history read APIs, project/team/iteration dashboard summary APIs, workspace/program/project-set dashboard summaries including import-completion metrics, custom field definition/context/value APIs, field configuration CRUD with project/type overrides, screen field/assignment APIs, current-user notification preference APIs, workspace notification default APIs with upsert coverage, direct notification creation/read coverage, automation rule/action/condition/job APIs, queued automation execution worker APIs, automation worker run/health/export/prune APIs, worker type/trigger/status/date-range run retention filtering, automatic worker-run pruning schedule settings, import conflict-resolution worker settings and run/health rows, scheduled import review export worker settings and run/health rows, workspace email provider settings, webhook config/delivery APIs with dry-run worker processing and dead-letter status, CSV/Jira/Rally import parser coverage, import job lifecycle/record APIs, import record edit/version-history/diff-row coverage, job-level import diff/export and stored JSON/CSV export artifact coverage, queued import review CSV export artifacts and manual/scheduled processing for conflict jobs/export jobs/completion metrics, import record lifecycle snapshots through parse/materialization/conflict changes, filtered record lists, import transform preset CRUD/JPA coverage, immutable transform preset version history, clone-from-version and clone-retarget coverage, transform validation, import materialization snapshot JPA/API coverage, mapping-rule snapshot persistence, exact and modified materialization rerun coverage, import idempotency skipped/conflicted counters, dedicated single/selected/filtered import conflict list/preview/resolve coverage with paginated preview safeguards, queued filtered conflict-resolution job cancel/retry/failure/worker coverage, guarded import completion with typed open-conflict confirmation and first-class audit/report/dashboard fields, full-flow sample payload fixture coverage for CSV/Jira/Rally, backend sample job/catalog/settings gating and generated mapping coverage, import mapping preset/template transformation pipelines/value/type/status rule materialization coverage, import activity/audit projection, saved dashboard/widget CRUD and render APIs, project/team-scoped dashboard, saved-filter, saved-view, and report-query list endpoints, dashboard/saved-filter/catalog/view sharing checks, saved filter CRUD, saved view/favorite/recent item APIs, governed report query catalog CRUD and runtime parameter validation, raw/report-shaped snapshot read APIs, live/snapshot iteration reports, reporting snapshot execution/backfill/reconcile, snapshot retention policy updates, manual reporting rollup/backfill execution, archive-run metadata, rollup-backed snapshot series output, S3-compatible attachment storage provider tests, activity stream projection, audit projection with redaction, audit retention policy updates, storage-backed export/prune/list/download, admin event replay idempotency, authenticated work item API flows with hierarchy validation, explicit team assignment command/history/events, ranking, transitions, assignment/team/estimate history, closure rows, rich JSON documents, comments, links, watchers, work logs, labels, attachment metadata, local attachment upload/download/delete cleanup, domain events, and the first simulated/Codex/Claude Code/generic-worker AI agent lifecycle with provider/profile/repository setup, runtime preview, project-scoped profiles, provider-specific Codex/Claude credential validation, env-gated backend-managed Codex/Claude CLI-worker execution using local command stand-ins, CLI run artifact list/download/delete/prune, encrypted per-provider signed callbacks, durable dispatch/retry/cancel attempt persistence, dispatch-attempt admin list/export/prune, automatic dispatch-attempt retention pruning, durable worker webhook dispatch, provider-level webhook dead-letter behavior, generic worker claim/heartbeat/log/message/artifact/cancel/retry flows, per-worker token credentials with expiry coverage, credential maintenance, immediate callback key invalidation on rotation, request-changes/human-message flows, agent permission denial, review artifacts/comments, callback idempotency, acceptance-to-Approval transition, activity projection, and audit projection.

Current external Playwright coverage in `../trasck-test` includes Java Playwright smoke tests for backend health, backend CSRF metadata, setup endpoint reachability and optional disposable first-run bootstrap, first-run-only setup conflict behavior, unauthenticated current-user denial, optional login/current-user cookie and Bearer sessions, local OpenAPI availability, opt-in managed production-like stack startup/security coverage for unsafe production defaults, unreachable Redis-backed rate limiting, protected OpenAPI/Swagger, secure auth cookies, system-admin list/grant/revoke gates, last-active-admin protection, recent-login step-up, and non-admin denial, committed route coverage baseline enforcement with `coverageOwner` on covered rows, generated route inventory output, authenticated API coverage for workspace/project security policy and public project/work-item/comment/attachment read boundaries with signed public attachment downloads, personal and service token scope/redaction/revocation behavior, system-admin/audit/export reads and mutations, domain-event replay, configuration/custom-field/context/field-configuration/screen/screen-field/screen-assignment/report-query/reporting-retention CRUD, work item custom field values, work item collaboration/activity, work item assignment/ranking/team/transition commands, attachment byte upload/download, workspace and work-item label cleanup, reporting history/snapshot reads and snapshot execution controls, program CRUD/project assignment/dashboard summary coverage, team-scoped dashboard/filter/view/report-query lists, repository connections, invitation list/cancellation, workspace-admin human user list/creation/removal, full workspace/project role management, teams/memberships/planning/boards/swimlanes/board moves/releases/roadmaps plus board status-option and iteration/team/view-mode scoped board-query coverage, notification preference/default CRUD, notification lists, direct notification creation, notification read state, automation-created notification read coverage, automation rule/job/worker settings coverage, signed local-receiver-backed webhook delivery coverage with dual-secret rotation overlap and durable delivery key IDs, Maildev dry-run email delivery coverage, worker run/health retention export/prune coverage, high-risk import lifecycle/review-export/report API coverage, auth/logout/register/OAuth unsafe-input coverage, provider-neutral agent provider/profile/credential/task/callback/worker-token/dispatch-attempt/CLI-run-artifact API coverage including Codex/Claude CLI provider readiness and credential metadata, dashboards/widgets, saved filters, saved views, favorites, and recent items when credentials plus workspace/project IDs are configured or setup bootstrap is enabled against an empty stack, resource cleanup through public APIs where cleanup APIs exist, optional sample-data fixture support separate from setup bootstrap, frontend shell, Workspace Settings member/invitation, program/portfolio management, automation async worker controls, auth/token/system-admin controls, import review/export worker controls, agent provider/profile/repository/task/dispatch-attempt/CLI-run controls, public-read settings, and public project/work-item/comment/attachment preview browser coverage, plus real-backend seeded browser workflows covering UI login, no browser access-token storage, work item load/create, saved-filter execution, dashboard render, team creation, custom-field creation, planning overview navigation, planning admin creation flows, backlog sprint assignment, sprint start, active sprint board rendering, board/release/roadmap detail loading, agent provider/profile/repository/task/dispatch-attempt flows with explicit provider/profile deactivation, work item collaboration creation/editing through the Work route UI, split feature-specific import, automation/worker, notification, configuration/search, dashboard, program/admin/token, public-preview, and collaboration workflows, and disposable first-run route rehearsal plus long planning/import/automation/agent scenario screenshots/traces/console-error capture. The checked-in CI workflow compiles the Java suite and runs the Docker-backed `managed-stack` production security group on pull requests and `main` pushes when the required `TRASCK_BACKEND_REPOSITORY` repository variable is set. The committed route baseline currently tracks 403 covered routes, 0 ordinary planned routes, and 0 planned high-risk routes. That repo is the planned home for full-stack API, browser, security, and cross-system regression coverage.
...
- Import job tests for idempotency, partial failure handling, record version history and backend diff rows, job-level diff exports/artifacts, review-table CSV export artifacts, filtered record lists, bulk conflict resolution, queued filtered conflict-resolution job cancel/retry/failure/worker behavior, scheduled worker-settings pickup with dedicated worker run/health history, clone-retarget behavior, guarded completion fields, sample payload and backend sample job flows, sample endpoint deployment gating, and snapshot reruns.
...
- Verifying no deployable secret, production credential, private key, token, SMTP password, OAuth secret, database password, object-storage secret, webhook secret, or agent credential is hard-coded in checked-in backend/frontend files, docs, config, Docker/nginx/build files, generated artifacts, tests, or sample data.
- Verifying `.env`/environment-variable driven configuration for all production secrets, with checked-in examples clearly limited to local non-production use.
- Verifying tenant isolation and permission denial across every workspace/project/team scoped API and frontend route.
