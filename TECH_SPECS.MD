...
15. Build the external Java Playwright automation suite in `../trasck-test`. The initial harness is implemented with Maven, Java Playwright, environment-backed backend/frontend base URLs, optional login credentials plus workspace/project IDs for authenticated coverage, optional first-run disposable setup bootstrap through `/api/v1/setup`, Docker support, reusable API/browser helpers, deterministic local-stack guidance, generated OpenAPI route inventory output, redacted API snippets, screenshots, traces, backend API smoke tests for health, CSRF metadata, setup endpoint reachability and first-run-only conflict behavior, unauthenticated current-user denial, optional login/current-user cookie and Bearer sessions, local OpenAPI availability, authenticated API coverage for workspace/project security policy, personal and service token scope behavior, system-admin/audit/export reads, work item collaboration, teams/planning/boards, dashboards, saved filters, saved views, favorites, and recent items, cleanup through public APIs, and a frontend browser shell smoke test. This suite should become the full-stack regression home for implemented backend APIs, frontend workflows, security audit checks, worker/import/export/agent flows, and cross-system scenarios.
...
Current external Playwright coverage in `../trasck-test` includes Java Playwright smoke tests for backend health, backend CSRF metadata, setup endpoint reachability and optional disposable first-run bootstrap, first-run-only setup conflict behavior, unauthenticated current-user denial, optional login/current-user cookie and Bearer sessions, local OpenAPI availability, generated route inventory output, authenticated API coverage for workspace/project security policy, personal and service token scope/redaction/revocation behavior, system-admin/audit/export reads, work item collaboration, teams/planning/boards, dashboards, saved filters, saved views, favorites, and recent items when credentials plus workspace/project IDs are configured or setup bootstrap is enabled against an empty stack, resource cleanup through public APIs, and frontend shell navigation with screenshots/traces/console-error capture. That repo is the planned home for full-stack API, browser, security, and cross-system regression coverage.
