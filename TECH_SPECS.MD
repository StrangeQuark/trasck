...
- `GET /api/v1/workspaces/{workspaceId}/automation-rules`, `POST /api/v1/workspaces/{workspaceId}/automation-rules`, `GET /api/v1/automation-rules/{ruleId}`, `PATCH /api/v1/automation-rules/{ruleId}`, `DELETE /api/v1/automation-rules/{ruleId}`, condition/action child routes, `POST /api/v1/automation-rules/{ruleId}/execute`, `GET /api/v1/automation-rules/{ruleId}/jobs`, and `GET /api/v1/automation-jobs/{jobId}` manage and execute the first automation rules.
- `GET /api/v1/workspaces/{workspaceId}/webhooks`, `POST /api/v1/workspaces/{workspaceId}/webhooks`, `PATCH /api/v1/webhooks/{webhookId}`, `DELETE /api/v1/webhooks/{webhookId}`, and `GET /api/v1/webhooks/{webhookId}/deliveries` manage webhook configuration and queued delivery records.
- `GET /api/v1/workspaces/{workspaceId}/import-jobs`, `POST /api/v1/workspaces/{workspaceId}/import-jobs`, `GET /api/v1/import-jobs/{importJobId}`, `POST /api/v1/import-jobs/{importJobId}/start`, `POST /api/v1/import-jobs/{importJobId}/complete`, `POST /api/v1/import-jobs/{importJobId}/fail`, `POST /api/v1/import-jobs/{importJobId}/cancel`, `GET /api/v1/import-jobs/{importJobId}/records`, and `POST /api/v1/import-jobs/{importJobId}/records` manage import job lifecycle shells and per-source import records.
...
- Automation rule, automation child configuration, manual execution, webhook configuration, and webhook delivery read APIs require workspace `automation.admin`. The first runner creates in-app notifications synchronously, records email actions as execution logs, and writes queued webhook delivery rows; external webhook/email delivery workers remain future work.
- Import job list/detail/lifecycle/record APIs require workspace `workspace.admin` and currently model generic import bookkeeping before a source-specific parser is enabled.
...
8. Implement boards, columns, iterations, releases, and roadmaps. Project-scoped board/column/swimlane CRUD, iteration CRUD, iteration scope add/remove, commit, close, carryover, release CRUD/scope, and workspace/project roadmap APIs are implemented.
...
11. Implement notifications, automation, webhooks, and integrations. Current-user notification and notification-preference APIs, automation rule/condition/action/job APIs, a first inline automation runner, webhook configuration plus queued delivery records, and import job lifecycle/record APIs are implemented. Real SMTP/webhook delivery workers and source-specific import parsers remain.
