...
59. Frontend components should use feature-specific API services and hooks around the generated `TrasckApiClient`; the generated TypeScript client and `openapi.json` stay committed as the reviewed contract snapshot while the API is moving quickly. Browser cookie-authenticated unsafe requests use Spring cookie CSRF and send `X-XSRF-TOKEN`. The Vite frontend uses route-based navigation rather than a single tabbed console shell, and the Project Settings route is the long-term home for project-admin settings as additional project-scoped configuration APIs are added.
...
- Setup, auth login/register/OAuth/login/logout, health, and public project/work-item/comment/attachment reads are explicitly permitted.
