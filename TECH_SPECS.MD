...
63. Import jobs support CSV, Jira JSON, and Rally JSON parsing into reviewable import records. Import mapping templates can materialize parsed records into work items through the same service path as normal work item creation/update, with reusable versioned workspace transform presets, first-class immutable transform preset version records, template-local transformation overrides, legacy transformation functions, argument-based transformation pipelines, value lookup rules, and type/status translation rules applied before materialization. Each materialization writes an immutable snapshot row containing the mapping template, mapping rules, transform preset version/config, merged transformation config, counts, status, actor, and target project. Import records have append-only lifecycle version snapshots for parse, edit, materialization, conflict-opened, and conflict-resolved changes, and the frontend computes adjacent field-level diff summaries for review. Import conflicts are first-class reviewable record state, `update_existing` conflict resolution stages the record as pending for a later materialization/rerun, selected bulk conflict resolution stays explicit by record ID, filtered bulk conflict resolution requires a preview count plus exact `RESOLVE FILTERED CONFLICTS` confirmation and matching `expectedCount`, transform preset historical versions can be cloned into standalone new presets or cloned and retargeted to selected mapping templates through separate preview/apply APIs, exact rerun from a materialization snapshot reuses the stored template/rules/transform snapshot by default while allowing an explicit admin `updateExisting` override for modified reruns, and import completion may proceed with open conflicts only when the caller sends the typed `COMPLETE WITH OPEN CONFLICTS` confirmation and an audit reason.
...
- Import job list/detail/lifecycle/parse/record/template/preset/version/materialization/conflict/rerun APIs require workspace `workspace.admin`. CSV, Jira JSON, and Rally JSON parsers create reviewable import records; admins can filter records by status/conflict/source type, edit imported records before materialization, and inspect append-only record version history. Mapping templates can create or update work items through the existing work item service after applying transform presets, template-local transformation config, value lookups, type translations, and status translations, preserving project permissions, sequence allocation, workflow defaults, screen/custom-field validation, history writes, and domain events. Conflict resolution can mark a record skipped, stage it for update-existing processing, or clear the target so a rerun can create a new work item; single-record, selected bulk, and filtered bulk resolution use the same validation, with filtered bulk guarded by preview, `expectedCount`, and typed confirmation. Completing a job with open conflicts requires `acceptOpenConflicts=true`, the `COMPLETE WITH OPEN CONFLICTS` phrase, and a non-empty audit reason.
...
- `import_jobs`, `import_transform_presets`, `import_mapping_templates`, import mapping rule tables, `import_materialization_runs`, `import_job_records`, and `import_job_record_versions` provide observable import progress, reusable source mappings, reusable transformation pipelines, transformation/translation rules, materialization output, exact rerun snapshots, conflict review, lifecycle history, and error reporting.
...
- Import job tests for idempotency, partial failure handling, record version history, filtered record lists, bulk conflict resolution, clone-retarget behavior, guarded completion, and snapshot reruns.
