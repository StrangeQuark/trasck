...
67. A full production-minded security audit has been recorded in `SECURITY_AUDIT.MD`. The current hardening baseline removes tracked `.env` files, adds committed examples only, rejects unsafe production defaults, gates Swagger/OpenAPI behind dedicated system-admin access in production-like profiles, exposes system-admin grant/revoke/list management, enforces explicit CORS, adds outbound egress/SSRF controls with exact host, wildcard host, and CIDR allowlisting, tightens login/password policy, supports database-backed or Redis-backed auth throttling while persisting auth-failure telemetry, rate-limits agent callback/worker auth failures, enforces workspace-admin configurable attachment/import/export size and content-type policy, masks frontend secret displays, centralizes redacted API error bodies/log messages while preserving explicit client-error status shapes, templates production nginx CSP, wires Java dependency vulnerability scanning as a required CI gate, and adds targeted regression tests. Remaining production hardening should focus on stronger secret-management/KMS abstraction, deciding whether Redis must be mandatory for production-like profiles, step-up safeguards for system-admin mutation, and broader redaction/tenant-isolation regression coverage. Production secrets must be injected through environment variables or local `.env` files that are not committed; checked-in config may contain only safe local examples.
...
- `GET /api/v1/system-admins`, `POST /api/v1/system-admins`, and `DELETE /api/v1/system-admins/{userId}` let active system admins list, grant, and revoke dedicated system-admin access while preserving at least one active system admin.
...
- The production-minded backend/frontend security sweep is recorded in `SECURITY_AUDIT.MD`. The current hardening baseline removes checked-in `.env` files in favor of `.env.example`, rejects unsafe production-like startup defaults, gates Swagger/OpenAPI behind dedicated system-admin access in production-like profiles, adds dedicated system-admin management APIs and frontend controls, enables explicit CORS, adds default-deny outbound URL/SSRF policy with HTTP timeouts and exact host, wildcard host, and CIDR allowlisting, tightens password/login protections, supports database-backed or Redis-backed auth throttling, persists auth-failure telemetry, rate-limits agent callback/worker auth failures, enforces workspace-admin configurable attachment/import/export size and content-type limits, masks frontend secret displays, centralizes redacted production API errors/log messages, templates production nginx CSP, wires Java dependency vulnerability scanning as a required CI gate, updates frontend container builds to `npm ci`, and adds baseline nginx security headers. Remaining production work includes stronger secret-management/KMS abstraction, deciding whether Redis must be mandatory in production-like profiles, system-admin step-up safeguards, and a broader dedicated security regression suite.
...
- New human passwords are validated for configured minimum length and common development password rejection during setup, invitation registration, and workspace-admin direct user creation. Login failures are throttled by identifier plus remote address with configurable database-backed or Redis-backed counters, failure window, and lockout duration; `X-Forwarded-For` is ignored unless `TRASCK_TRUST_FORWARDED_FOR=true` is set behind a trusted proxy. Login, agent callback, and generic worker authentication failures write durable security telemetry rows.
...
- Attachment metadata/upload/download, generated export artifacts/downloads, and import parser payloads enforce effective workspace security policy byte limits and allowlisted content types. Workspace admins can override deployment defaults per workspace; multipart request size limits remain environment-configurable.
- Activity streams use service-layer permission checks for the requested workspace, project, or work item.
- System-admin list/grant/revoke APIs require active dedicated system-admin access and refuse to remove the last active system admin. Workspace security policy APIs require workspace `workspace.admin`. Audit log, audit retention, and domain event replay APIs are workspace-admin-only through method-level permission checks.
