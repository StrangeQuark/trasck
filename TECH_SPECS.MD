...
- `GET /api/v1/workspaces/{workspaceId}/security-policy` and `PATCH /api/v1/workspaces/{workspaceId}/security-policy` let workspace admins inspect and update effective attachment, import, and export size/content-type policies.
- `GET /api/v1/projects/{projectId}/security-policy` and `PATCH /api/v1/projects/{projectId}/security-policy` let project admins inspect inherited workspace limits and apply project-level attachment, import, and export size/content-type overrides.
...
15. Build the external Java Playwright automation suite in `../trasck-test`. The initial harness is implemented with Maven, Java Playwright, environment-backed backend/frontend base URLs, optional login credentials plus workspace/project IDs for authenticated coverage, optional first-run disposable setup bootstrap through `/api/v1/setup`, separate opt-in sample-data fixture support, Docker support, reusable API/browser helpers, deterministic local-stack guidance, committed OpenAPI route coverage baseline enforcement with per-covered-route `coverageOwner`, generated OpenAPI route inventory output, redacted API snippets, screenshots, traces, backend API smoke tests for health, CSRF metadata, setup endpoint reachability and first-run-only conflict behavior, unauthenticated current-user denial, optional login/current-user cookie and Bearer sessions, local OpenAPI availability, authenticated API coverage for workspace/project security policy, personal and service token scope behavior, system-admin/audit/export reads, configuration/custom-field/screen/report-query/reporting-retention CRUD, work item collaboration/activity including label cleanup and attachment byte upload/download, reporting history/snapshot reads and snapshot execution controls, team-scoped dashboard/filter/view/report-query lists, repository connections, invitations, workspace-admin user creation, teams/planning/boards/releases/roadmaps, notification preferences/defaults, dashboards, saved filters, saved views, favorites, and recent items, cleanup through public APIs where cleanup APIs exist, and a frontend browser shell smoke test. This suite should become the full-stack regression home for implemented backend APIs, frontend workflows, security audit checks, worker/import/export/agent flows, and cross-system scenarios. The committed route baseline currently tracks 212 covered routes, 3 ordinary planned routes, and 144 planned high-risk routes.
...
Current external Playwright coverage in `../trasck-test` includes Java Playwright smoke tests for backend health, backend CSRF metadata, setup endpoint reachability and optional disposable first-run bootstrap, first-run-only setup conflict behavior, unauthenticated current-user denial, optional login/current-user cookie and Bearer sessions, local OpenAPI availability, committed route coverage baseline enforcement with `coverageOwner` on covered rows, generated route inventory output, authenticated API coverage for workspace/project security policy, personal and service token scope/redaction/revocation behavior, system-admin/audit/export reads, configuration/custom-field/context/field-configuration/screen/screen-field/screen-assignment/report-query/reporting-retention CRUD, work item custom field values, work item collaboration/activity, work item assignment/ranking/team/transition commands, attachment byte upload/download, workspace and work-item label cleanup, reporting history/snapshot reads and snapshot execution controls, team-scoped dashboard/filter/view/report-query lists, repository connections, invitations, workspace-admin user creation, teams/memberships/planning/boards/swimlanes/board moves/releases/roadmaps, notification preference/default CRUD and notification lists, dashboards/widgets, saved filters, saved views, favorites, and recent items when credentials plus workspace/project IDs are configured or setup bootstrap is enabled against an empty stack, resource cleanup through public APIs where cleanup APIs exist, optional sample-data fixture support separate from setup bootstrap, and frontend shell navigation with screenshots/traces/console-error capture. The committed route baseline currently tracks 212 covered routes, 3 ordinary planned fixture-gated routes, and 144 planned high-risk routes. That repo is the planned home for full-stack API, browser, security, and cross-system regression coverage.
