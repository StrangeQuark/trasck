...
- `GET /api/v1/public/projects/{projectId}/work-items/{workItemId}/attachments` returns public attachment metadata for a public work item. Public attachment responses omit storage configuration IDs, storage keys, and uploader IDs, and include a short-lived app-signed backend `downloadUrl`.
...
15. Build the external Java Playwright automation suite in `../trasck-test`. The initial harness is implemented with Maven, Java Playwright, environment-backed backend/frontend base URLs, optional login credentials plus workspace/project IDs for authenticated coverage, optional first-run disposable setup bootstrap through `/api/v1/setup`, separate opt-in sample-data fixture support, Docker support, reusable API/browser helpers, deterministic local-stack guidance, committed OpenAPI route coverage baseline enforcement with per-covered-route `coverageOwner`, generated OpenAPI route inventory output, redacted API snippets, screenshots, traces, backend API smoke tests for health, CSRF metadata, setup endpoint reachability and first-run-only conflict behavior, unauthenticated current-user denial, optional login/current-user cookie and Bearer sessions, local OpenAPI availability, authenticated API coverage for workspace/project security policy and public project/work-item/comment/attachment read boundaries with signed public attachment downloads, personal and service token scope behavior, system-admin/audit/export reads, configuration/custom-field/screen/report-query/reporting-retention CRUD, work item collaboration/activity including label cleanup and attachment byte upload/download, reporting history/snapshot reads and snapshot execution controls, program CRUD/project assignment/dashboard summary coverage, team-scoped dashboard/filter/view/report-query lists, repository connections, invitation list/cancellation, workspace-admin human user list/creation/removal, full workspace/project role management, teams/planning/boards/releases/roadmaps, notification preferences/defaults plus direct and automation-created notification read coverage, dashboards, saved filters, saved views, favorites, and recent items, cleanup through public APIs where cleanup APIs exist, and frontend browser smoke tests for the shell, Workspace Settings member/invitation flow, program/portfolio management, public-read settings, and public project/work-item/comment/attachment preview route. This suite should become the full-stack regression home for implemented backend APIs, frontend workflows, security audit checks, worker/import/export/agent flows, and cross-system scenarios. The committed route baseline currently tracks 253 covered routes, 0 ordinary planned routes, and 144 planned high-risk routes.
