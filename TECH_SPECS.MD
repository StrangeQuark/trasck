...
11. Implement notifications, automation, webhooks, and integrations. Current-user notification/preference APIs, workspace default notification policy APIs, automation rule/condition/action/job APIs, queued automation worker endpoints, worker run history/health/export/prune APIs, optional automatic worker-run pruning with dedicated interval/window throttling, webhook configuration plus queued delivery records, webhook delivery processing with retry/dead-letter, workspace Maildev/SMTP email provider settings, workspace-provider-backed email delivery rows, import job lifecycle/record/parser/edit APIs for CSV/Jira/Rally, import record lifecycle version snapshots and normalized backend diff rows, job-level import diff/export APIs plus stored export-job artifacts, reusable versioned import transform presets with immutable version records, clone-from-version recovery, clone-and-retarget preview/apply, import materialization with preset/template transformation pipelines, mapping-rule snapshots, exact and modified rerun from materialization snapshots, skipped/conflicted idempotency counters, filtered record lists, dedicated single/selected/filtered conflict review/resolve APIs with paginated preview and batch caps, queued filtered conflict-resolution jobs for larger sets with cancel/retry/manual run/bounded worker pickup, workspace-settings-controlled scheduled conflict-resolution pickup with dedicated worker run/health rows, focused import completion report endpoints/widgets, guarded completion with typed open-conflict confirmation and first-class audit/report/dashboard fields, executable full-flow sample import fixtures, deployment-gated backend-generated sample import jobs/mappings, sample import payloads, and value/type/status mapping rules are implemented.
