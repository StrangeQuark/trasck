...
| `automation_worker_settings` | `workspace_id`, `automation_jobs_enabled`, `webhook_deliveries_enabled`, `email_deliveries_enabled`, worker limits, max attempts, dry-run switches, worker run retention settings, `worker_run_pruning_automatic_enabled`, `worker_run_pruning_interval_minutes`, pruning window and last-run timestamps, `updated_at` |
...
- `import_jobs`, `import_conflict_resolution_jobs`, `import_transform_presets`, `import_mapping_templates`, import mapping rule tables, `import_materialization_runs`, `import_job_records`, and `import_job_record_versions` provide observable import progress, reusable source mappings, reusable transformation pipelines, transformation/translation rules, materialization output, exact rerun snapshots, conflict review, queued large filtered conflict resolution, completion-audit/reporting fields, lifecycle history, normalized record/job diff rows, and error reporting.
...
11. Implement notifications, automation, webhooks, and integrations. Current-user notification/preference APIs, workspace default notification policy APIs, automation rule/condition/action/job APIs, queued automation worker endpoints, worker run history/health/export/prune APIs, optional automatic worker-run pruning with dedicated interval/window throttling, webhook configuration plus queued delivery records, webhook delivery processing with retry/dead-letter, workspace Maildev/SMTP email provider settings, workspace-provider-backed email delivery rows, import job lifecycle/record/parser/edit APIs for CSV/Jira/Rally, import record lifecycle version snapshots and normalized backend diff rows, job-level import diff/export APIs plus stored export-job artifacts, reusable versioned import transform presets with immutable version records, clone-from-version recovery, clone-and-retarget preview/apply, import materialization with preset/template transformation pipelines, mapping-rule snapshots, exact and modified rerun from materialization snapshots, skipped/conflicted idempotency counters, filtered record lists, dedicated single/selected/filtered conflict review/resolve APIs with paginated preview and batch caps, queued filtered conflict-resolution jobs for larger sets with cancel/retry/manual run/bounded worker pickup, focused import completion report endpoints/widgets, guarded completion with typed open-conflict confirmation and first-class audit/report/dashboard fields, executable full-flow sample import fixtures, sample import payloads, and value/type/status mapping rules are implemented.
...
- Import job tests for idempotency, partial failure handling, record version history and backend diff rows, job-level diff exports/artifacts, filtered record lists, bulk conflict resolution, queued filtered conflict-resolution job cancel/retry/failure/worker behavior, clone-retarget behavior, guarded completion fields, sample payload flows, and snapshot reruns.
