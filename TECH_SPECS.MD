...
64. Boards expose per-column work item card data from configured status mappings plus configured backend swimlane groupings backed by the saved-filter/query-builder predicate engine. Swimlanes may reference saved filter IDs directly or store inline query JSON. Board-scoped rank, transition, and combined move commands validate board membership before using the normal work item command paths; transition/move commands can use explicit `transitionKey` or derive an allowed workflow transition from `targetColumnId` and/or `targetStatusId`. Drag/drop uses card-level insertion by sending `previousWorkItemId`/`nextWorkItemId` relative to the target column. This board engine is the underlying planning foundation, but the product-facing planning UX should still grow into a true Jira/Rally-style backlog plus active sprint board flow that is filtered by iteration/team scope rather than exposing only board-configuration-centric status groupings.
...
- `GET /api/v1/boards/{boardId}/work-items` returns per-column work item cards using configured column status mappings and a bounded `limitPerColumn`, plus configured swimlane groupings over those columns. Swimlanes can either reference a readable saved filter ID or store inline query JSON that reuses the saved-filter/query-builder predicate shape.
