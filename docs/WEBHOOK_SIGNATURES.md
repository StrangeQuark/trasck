...
Creating a webhook secret assigns a `secretKeyId` in the webhook response. Updating the webhook with a new `secret` rotates the encrypted signing secret and issues a new `secretKeyId`. Trasck keeps the immediately previous encrypted secret, previous key ID, rotation timestamp, and previous-secret expiry metadata for a configurable overlap window. Each webhook can set `previousSecretOverlapSeconds`; when omitted, Trasck uses the deployment default. The default is 72 hours and can be changed with the positive ISO-8601 duration property `trasck.webhooks.previous-secret-overlap` or environment variable `TRASCK_WEBHOOK_PREVIOUS_SECRET_OVERLAP`.
