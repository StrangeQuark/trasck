...
- `TRASCK_CODEX_AGENT_COMMAND`, `TRASCK_CODEX_AGENT_ARGS`, `TRASCK_CODEX_AGENT_WORKDIR`, `TRASCK_CODEX_AGENT_SEND_PROMPT_TO_STDIN`, and `TRASCK_CODEX_AGENT_CREDENTIAL_TYPES`: define the allowlisted `codex-local` command profile. The default manual-test command shape is `codex exec --full-auto -`, with the Trasck prompt sent to stdin.
- `TRASCK_CLAUDE_CODE_AGENT_COMMAND`, `TRASCK_CLAUDE_CODE_AGENT_ARGS`, `TRASCK_CLAUDE_CODE_AGENT_WORKDIR`, `TRASCK_CLAUDE_CODE_AGENT_SEND_PROMPT_TO_STDIN`, and `TRASCK_CLAUDE_CODE_AGENT_CREDENTIAL_TYPES`: define the allowlisted `claude-code-local` command profile. The default manual-test command shape is `claude -p {prompt} --permission-mode bypassPermissions`.
...
The local defaults are intentionally edit-capable because the first human test round needs full Codex capability and a higher-permission Claude Code run. Keep `TRASCK_AGENTS_CLI_WORKER_ENABLED=false` except on a trusted worker host, point `TRASCK_CODEX_AGENT_WORKDIR` and `TRASCK_CLAUDE_CODE_AGENT_WORKDIR` at disposable checkouts, and inspect the generated task before accepting results. To force read-only behavior for a specific run, override `TRASCK_CODEX_AGENT_ARGS` or `TRASCK_CLAUDE_CODE_AGENT_ARGS` with a lower-permission command profile.

Backend-generated run directories can be administered from the frontend Agent page under `CLI Run Artifacts`. Workspace admins can list run metadata, download a ZIP archive for completed or review-requested runs, delete a single run directory, or prune old non-active run directories by retention days. The same controls are available through `GET /api/v1/workspaces/{workspaceId}/agent-cli-runs`, `GET /api/v1/workspaces/{workspaceId}/agent-cli-runs/{agentTaskId}/download`, `DELETE /api/v1/workspaces/{workspaceId}/agent-cli-runs/{agentTaskId}`, and `POST /api/v1/workspaces/{workspaceId}/agent-cli-runs/prune`.
