...
Override these with `TRASCK_BACKEND_BASE_URL`, `TRASCK_FRONTEND_BASE_URL`, `TRASCK_E2E_BROWSER`, `TRASCK_E2E_HEADLESS`, and `TRASCK_E2E_TIMEOUT_MS`. Authenticated API coverage also supports `TRASCK_E2E_LOGIN_IDENTIFIER`, `TRASCK_E2E_LOGIN_PASSWORD`, `TRASCK_E2E_WORKSPACE_ID`, and `TRASCK_E2E_PROJECT_ID`; those tests create uniquely named resources and clean them up through public APIs. For a fresh disposable local database, `TRASCK_E2E_ALLOW_SETUP=true` lets the suite create the first admin/workspace/project through `/api/v1/setup` and verify that a second setup call is rejected. The test repo also writes a generated OpenAPI route inventory to `test-results/api/backend-route-inventory.tsv` and supports `docker compose run --rm trasck-test` for running the Java Playwright suite from its container against services on the host.
