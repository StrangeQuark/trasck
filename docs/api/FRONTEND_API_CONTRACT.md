...
- Import sample job endpoints are workspace-admin-only. They are available in local/default runtime profiles, but production-like `prod`, `production`, `staging`, and `hosted` profiles require `TRASCK_IMPORT_SAMPLE_JOBS_ENABLED=true`.
...
export interface AutomationWorkerRun {
  id: UUID;
  workspaceId: UUID;
  workerType: "automation" | "webhook" | "email" | "import_conflict_resolution" | string;
